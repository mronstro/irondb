...
int runCreateInvalidTables(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  const int expectedDictErrors[6] = {720, 4317, 737, 739, 736, 740};
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (pFailTab != NULL) {
      ...
      if (pTab2 != NULL) {
        ndbout << failTabName << " was found in DB, this was not expected"
               << endl;
        result = NDBT_FAILED;
...
int runCreateTheTable(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  BaseString::snprintf(f_tablename, sizeof(f_tablename), "%s", pTab->getName());
...
int runCreateAndDropAtRandom(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops && result == NDBT_OK) {
    ...
    if (tabList[num].exists == -1) continue;
    ...
    if (tabList[num].exists == 0) {
      if (bias == 0 && myRandom48(100) < 80) continue;
      ...
      if (createIndexes) {
        ...
        for (inum = 0; inum < icount; inum++) {
          ...
          for (ic = 0; ic < icols; ic++) {
            ...
            require(c != 0);
            if (mask.get(tc) || c->getType() == NdbDictionary::Column::Blob ||
      ...
      if (loadData) {
        ...
        while (1) {
          ...
          if (num == num3 || tabList[num3].exists == 1) break;
        }
      ...
      numExists++;
      if (numExists == numTables) bias = 0;
    } else if (tabList[num].exists == 1) {
      if (bias == 1 && myRandom48(100) < 80) continue;
      ...
      if (pDic->getNdbError().code != 709 && pDic->getNdbError().code != 723) {
        const NdbError err = pDic->getNdbError();
...
int runUseTableUntilStopped2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (pTab2 == NULL) {
    g_err << "Table: " << pTab->getName() << ", not 'discovered' on line "
          << __LINE__ << endl;
    return NDBT_FAILED;
  }
  HugoTransactions hugoTrans(*pTab2);
 ...
 * multi-threaded ndb.
 */
static int runDropTakeoverTest(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  ...
  const int nonMasterNodeId = masterNodeId == restarter.getDbNodeId(0)
...
int runBackup(NDBT_Context *ctx, NDBT_Step *step) {
  NdbBackup backup;
...
int runCreateMaxTables(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (i = 0; i < numTables; i++) {
    ...
    if (newTab.createTableInDb(pNdb) != 0) {
      ndbout << tabName << " could not be created: " << pDic->getNdbError()
             << endl;
      if (pDic->getNdbError().code == 625 || pDic->getNdbError().code == 707 ||
          pDic->getNdbError().code == 708 || pDic->getNdbError().code == 826 ||
          pDic->getNdbError().code == 827 || pDic->getNdbError().code == 921)
        break;
      return NDBT_FAILED;
    }
    ...
    const NdbDictionary::Table *pTab3 =
        NDBT_Table::discoverTableFromDb(pNdb, tabName);
    if (pTab3 == NULL) {
      ndbout << tabName << " was not found in DB: " << pDic->getNdbError()
             << endl;
      return NDBT_FAILED;
    }
...
int runDropMaxTables(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int i = 0; i < numTables; i++) {
    ...
    if (pDic->dropTable(pTab3->getName()) != 0) {
      ndbout << tabName << " could not be dropped: " << pDic->getNdbError()
             << endl;
      return NDBT_FAILED;
    }
  }
...
int runStoreExtraMetada(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int l = 0; l < ctx->getNumLoops() && result == NDBT_OK; l++) {
    ...
    const Uint32 dataLen =
        (testBufferLenInWords * 3) + myRandom48(testBufferLenInWords);
    ...
    */
    ...
    if (newTab.setExtraMetadata(version, &data, dataLen)) {
    ...
    void *data2;
    Uint32 dataLen2;
    if (pTab2->getExtraMetadata(version2, &data2, &dataLen2)) {
      result = NDBT_FAILED;
    }
...
int verifyTablesAreEqual(const NdbDictionary::Table *pTab,
                         const NdbDictionary::Table *pTab2) {
...
int runGetPrimaryKey(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int result = NDBT_OK;
  if (verifyTablesAreEqual(pTab, pTab2) != NDBT_OK) result = NDBT_FAILED;
...
int runCreateAutoincrementTable(NDBT_Context *ctx, NDBT_Step *step) {
  Uint32 startvalues[5] = {256 - 2, 0, 256 * 256 - 2, ~Uint32(0),
                           256 * 256 * 256 - 2};
...
int runTableRename(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  ...
  for (int l = 0; l < loops && result == NDBT_OK; l++) {
    ...
    if (oldTable) {
      ...
      CHECK2(dict->alterTable(*oldTable, newTable) == 0, "TableRename failed");
    } else {
      result = NDBT_FAILED;
    }
...
int runTableRenameSR(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  ...
  for (int l = 0; l < loops && result == NDBT_OK; l++) {
    ...
    CHECK2(restarter.restartAll() == 0, "failed to set restartOneDbNode");
...
/*
  Run rename column
*/
int runColumnRename(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  ...
  for (int l = 0; l < loops && result == NDBT_OK; l++) {
    ...
    */
    ...
    ndbout << "Column rename completed" << endl;
    {
      ...
      const NdbDictionary::Column *addedCol =
          pTab->getColumn("renamed_NEWKOL1");
      CHECK2(
          (addedCol) && addedCol->getColumnNo() == pTab->getNoOfColumns() - 1,
          "runColumnRename failed");
...
*/
int runColumnRenameSR(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  ...
  for (int l = 0; l < loops && result == NDBT_OK; l++) {
    ...
    */
    ...
    ndbout << "Column rename completed" << endl;
    {
      ...
      CHECK2(res.restartAll(false, true, false) == 0, "restart all failed");
      CHECK2(res.waitClusterNoStart() == 0, "waitClusterNoStart failed");
      CHECK2(res.startAll() == 0, "startAll failed");
      CHECK2(res.waitClusterStarted() == 0, "waitClusterStarted failed");
 ...
 */
int runTableAddAttrs(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  ...
  NdbDictionary::Table myTab = *(ctx->getTab());
  const bool testMaxRecSz = (ctx->getProperty("checkMaxRecSz", Uint32(0)) != 0);
 ...
 */
int runTableAddAttrsDuring(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
    ...
    */
    ...
    if (oldTable) {
      ...
      if (abortAlter == 0)
      {
        ...
        ndbout << "Start alterTable with abortAlter, inject error on node: "
               << nodeId << endl;
        CHECK2(dict->alterTable(*oldTable, newTable) != 0,
               "TableAddAttrsDuring failed");
      }
    ...
    if (nodeId) {
      ndbout << "Clear ErrorInsert on node " << nodeId << endl;
      CHECK2(res.insertErrorInNode(nodeId, 0) == 0,
             "failed to clear error insert");
      NdbSleep_MilliSleep(SAFETY);
    }
  }
end:
...
static void f(const NdbDictionary::Column *col) {
  if (col == 0) {
    abort();
  }
}

int runTestDictionaryPerf(NDBT_Context *ctx, NDBT_Step *step) {
  Vector<char *> cols;
  ...
  ndbout_c("%d tables and %d columns", NDBT_Tables::getNumTables(),
           cols.size() / 2);
  ...
  ndbout_c("%d random getColumn(name) in %lld ms -> %u us/get", times, stop,
           Uint32(per));

  return NDBT_OK;
}

int runCreateLogfileGroup(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::LogfileGroup lg;
  lg.setName("DEFAULT-LG");
  lg.setUndoBufferSize(8 * 1024 * 1024);

  int oneDictParticipantFail =
      ctx->getProperty("OneDictParticipantFail", (Uint32)0);
  ...
  if (res != 0) {
    g_err << "Failed to create logfilegroup:" << endl
  ...
  if (res != 0) {
    g_err << "Failed to create undofile:" << endl
          << pNdb->getDictionary()->getNdbError() << endl;
    return NDBT_FAILED;
  }
...
int runCreateTablespace(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  ...
  if (res != 0) {
    g_err << "Failed to create tablespace:" << endl
          << pNdb->getDictionary()->getNdbError() << endl;
    return NDBT_FAILED;
  }
  ...
  if (res != 0) {
    g_err << "Failed to create datafile:" << endl
          << pNdb->getDictionary()->getNdbError() << endl;
    return NDBT_FAILED;
  }
  return NDBT_OK;
...
int runDropTableSpaceLogFileGroup(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  ...
  if (pNdb->getDictionary()->dropLogfileGroup(
    ...
    g_err << "Error: Drop of LFG Failed" << endl
          << pNdb->getDictionary()->getNdbError() << endl;
    return NDBT_FAILED;
  }
  return NDBT_OK;
...
int runCreateDiskTable(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  ...
  if (res != 0) {
    g_err << "Failed to create table:" << endl
          << pNdb->getDictionary()->getNdbError() << endl;
    return NDBT_FAILED;
  }
...
int runFailAddFragment(NDBT_Context* ctx, NDBT_Step* step){
  ...
  for (int colNum = 0; colNum < tab.getNoOfColumns(); colNum++) {
    ...
    if (!origCol->getPrimaryKey()) {
      if (myRandom48(2) == 0) {
        ...
        const char *def =
            calc.calcValue(0, colNum, updatesVal, defaultBuf,
                           getColumnMaxLength(origCol), &real_len);
        if (col->setDefaultValue(def, real_len) != 0) {
          ndbout_c("Error setting default value\n");
          return NDBT_FAILED;
        }
        NdbDictionary::NdbDataPrintFormat dpf;
        ndbout << "Set default for column " << origCol->getName() << " to ";

        NdbDictionary::printFormattedValue(ndbout, dpf, col, def);
        ndbout << endl;
      }
    }
  }
  ...
  for (int cnt = 0, i_hate_broken_compilers = 0;
       cnt < 3 && i_hate_broken_compilers < tab.getNoOfColumns();
  ...
  for (int l = 0; l < loops; l++) {
    for (unsigned i0 = 0; i0 < acccnt; i0++) {
      ...
      int errval = acclst[j];
      if (errNo != 0 && errNo != errval) continue;
      ...
      NdbSleep_MilliSleep(SAFETY);  // Hope that snapshot has arrived
      ...
      CHECK2(pDic->dropTable(tab.getName()) == 0, pDic->getNdbError());
    }
    ...
    for (unsigned i2 = 0; i2 < tuxcnt; i2++) {
      ...
      CHECK(restarter.dumpStateAllNodes(&dump2, 1) == 0);
      NdbSleep_MilliSleep(SAFETY);  // Hope that snapshot has arrived
      CHECK2(pDic->createIndex(idx) == 0, pDic->getNdbError());
      CHECK2(pDic->dropTable(tab.getName()) == 0, pDic->getNdbError());
    }
  }
end:
...
static bool send_dict_ops_cmd(NDBT_Context *ctx, Uint32 cmd) {
  ctx->setProperty("DictOps_CMD", cmd);
  while (1) {
    if (ctx->isTestStopped()) return false;
    if (ctx->getProperty("DictOps_ACK") == cmd) break;
    NdbSleep_MilliSleep(100);
  }
  return true;
}

static bool recv_dict_ops_run(NDBT_Context *ctx) {
  while (1) {
    if (ctx->isTestStopped()) return false;
...
int runRestarts(NDBT_Context *ctx, NDBT_Step *step) {
  static int errlst_master[] = {       // non-crashing
                                ...
                                0};
  static int errlst_node[] = {7174,  // crash before sending DICT_LOCK_REQ
  ...
  for (int l = 0; l < loops && result == NDBT_OK; l++) {
    ...
    if (numnodes >= 4 && (myRandom48(2) == 0) &&
        (restarter.getNumNodeGroups() > 1)) {
      int rand = myRandom48(numnodes);
      int nodeId = restarter.getRandomNodeOtherNodeGroup(nodeIdList[0], rand);
      CHECK(nodeId != -1);
      if (nodeId != masterNodeId) nodeIdList[nodeIdCnt++] = nodeId;
    }

    g_info << "1: master=" << masterNodeId << " nodes=" << nodeIdList[0] << ","
           << nodeIdList[1] << endl;
    ...
    if (!send_dict_ops_cmd(ctx, NF_ops ? 1 : 2)) break;
...

    {
      for (int i = 0; i < nodeIdCnt; i++) {
        ...
        if (abort) strcat(flags, ",abort");
        if (initial) strcat(flags, ",initial");
    ...
    if (!send_dict_ops_cmd(ctx, NR_ops ? 1 : 2)) break;
    NdbSleep_MilliSleep(myRandom48(maxsleep));
...
int runDictOps(NDBT_Context *ctx, NDBT_Step *step) {
  myRandom48Init((long)NdbTick_CurrentMillisecond());
  int result = NDBT_OK;

  for (int l = 0; result == NDBT_OK; l++) {
    if (!recv_dict_ops_run(ctx)) break;
    ...
    try_drop_index:
      count++;
      if (count == 1) g_info << "2: drop index " << indnum << endl;
      char indName[200];
      sprintf(indName, "%s_X%u", tabName, indnum);
...
int runBug21755(NDBT_Context *ctx, NDBT_Step *step) {
  char buf[256];
  NdbRestarter res;
  NdbDictionary::Table pTab0 = *ctx->getTab();
  NdbDictionary::Table pTab1 = pTab0;

  if (res.getNumDbNodes() < 2) return NDBT_OK;
...

  {
    ...
    if (pTab == NULL) {
      g_err << "Table 'pTab0': " << pTab0.getName() << ", not found on line "
            << __LINE__ << ", error: " << pDic->getNdbError() << endl;
      return NDBT_FAILED;
    }
...

  {
    ...
    if (pTab == NULL) {
      g_err << "Table 'pTab1': " << pTab1.getName() << ", not found on line "
            << __LINE__ << ", error: " << pDic->getNdbError() << endl;
      return NDBT_FAILED;
    }
...
static int create_tablespace(NdbDictionary::Dictionary *pDict,
                             const char *lgname, const char *tsname,
                             const char *dfname) {
...
int runBug24631(NDBT_Context *ctx, NDBT_Step *step) {
  char tsname[256];
  char dfname[256];
  char lgname[256];
  char ufname[256];
  ...
  if (pDict->listObjects(list) == -1) return NDBT_FAILED;
  ...
  if (create_tablespace(pDict, lgname, tsname, dfname)) return NDBT_FAILED;
...
int runBug29186(NDBT_Context *ctx, NDBT_Step *step) {
  int lgError = 15000;
  int tsError = 16000;
  char lgname[256];
  char ufname[256];
  ...
  if (pDict->createUndofile(uf) == 0) {
    g_err << "Create log file group should fail on error_insertion " << lgError
          << endl;
    return NDBT_FAILED;
  }
...
int RandSchemaOp::schema_op(Ndb *ndb) {
  struct Obj *obj = 0;
  Uint32 type = 0;
loop:
  switch (ndb_rand_r(seed) % 5) {
    ...
    case 2:
      ...
      goto drop_object;
    case 3:
      type = (1 << NdbDictionary::Object::UniqueHashIndex) |
...
drop_object:
  if ((obj = get_obj(type)) == 0) goto loop;
...
RandSchemaOp::Obj *RandSchemaOp::get_obj(Uint32 mask) {
  ...
  for (Uint32 i = 0; i < m_objects.size(); i++) {
    if ((1 << m_objects[i]->m_type) & mask) tmp.push_back(m_objects[i]);
  }
...
int RandSchemaOp::create_table(Ndb *ndb) {
...
int RandSchemaOp::create_index(Ndb *ndb, Obj *tab) {
  NdbDictionary::Dictionary *pDict = ndb->getDictionary();
  ...
  Uint32 type = ordered ? NdbDictionary::Index::OrderedIndex
                        : NdbDictionary::Index::UniqueHashIndex;

  char buf[255];
  BaseString::snprintf(buf, sizeof(buf), "%s-%s", pTab->getName(),
                       ordered ? "OI" : "UI");
...
int RandSchemaOp::drop_obj(Ndb *ndb, Obj *obj) {
  NdbDictionary::Dictionary *pDict = ndb->getDictionary();
...
void RandSchemaOp::remove_obj(Obj *obj) {
  Uint32 i;
  if (obj->m_parent) {
    bool found = false;
...
int RandSchemaOp::alter_table(Ndb *ndb, Obj *obj) {
  NdbDictionary::Dictionary *pDict = ndb->getDictionary();
...
int RandSchemaOp::validate(Ndb *ndb) {
  NdbDictionary::Dictionary *pDict = ndb->getDictionary();
  for (Uint32 i = 0; i < m_objects.size(); i++) {
...
int RandSchemaOp::cleanup(Ndb *ndb) {
  Int32 i;
  for (i = m_objects.size() - 1; i >= 0; i--) {
    switch (m_objects[i]->m_type) {
      case NdbDictionary::Object::UniqueHashIndex:
      case NdbDictionary::Object::OrderedIndex:
        if (drop_obj(ndb, m_objects[i])) return NDBT_FAILED;
...
int runDictRestart(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  int loops = ctx->getNumLoops();
  ...
  for (int i = 0; i < loops; i++) {
    ...
    if (dict.validate(pNdb)) return NDBT_FAILED;
  }

  if (res.finish(ctx, step)) return NDBT_FAILED;

  if (dict.validate(pNdb)) return NDBT_FAILED;

  if (dict.cleanup(pNdb)) return NDBT_FAILED;

  return NDBT_OK;
}

int runBug29501(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;
  NdbDictionary::LogfileGroup lg;
  lg.setName("DEFAULT-LG");
  lg.setUndoBufferSize(8 * 1024 * 1024);
  ...
  if (res.waitClusterStarted()) {
    g_err << "Node restart failed" << endl << pDict->getNdbError() << endl;
    return NDBT_FAILED;
  }
  CHK_NDB_READY(pNdb);

  if (pDict->dropLogfileGroup(pDict->getLogfileGroup(lg.getName())) != 0) {
    g_err << "Drop of LFG Failed" << endl << pDict->getNdbError() << endl;
    return NDBT_FAILED;
  }
...
int runDropDDObjects(NDBT_Context *ctx, NDBT_Step *step) {
        ...
        if (tableFound != 0) {
          if (strcmp(list.elements[i].database, "TEST_DB") == 0 &&
            ...
            if (pDict->dropTable(tableFound) != 0) {
              g_err << "Failed to drop table: " << tableFound
                    << pDict->getNdbError() << endl;
              return NDBT_FAILED;
            }
          }
...
int testDropDDObjectsSetup(NDBT_Context *ctx, NDBT_Step *step) {
...
int runBug36072(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
  NdbRestarter res;
  ...
  for (Uint32 i = 0; err[i] != 0; i++) {
    ...
    if (code == 6016) {
      if (res.insertErrorInAllNodes(code)) return NDBT_FAILED;
    }
    ...
    if (code == 6017) {
      ...
      if (res.insertErrorInAllNodes(err[i])) return NDBT_FAILED;
...
int restartClusterInitial(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;

  res.restartAll2(NdbRestarter::NRRF_INITIAL | NdbRestarter::NRRF_NOSTART |
...
int DropDDObjectsVerify(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (i = 0; i < list.count; i++) {
    ...
    if (ddFound == true) {
      g_err << "DropDDObjects Failed: DD found:" << endl;
      return NDBT_FAILED;
    }
  }
  return NDBT_OK;
...
static bool send_msg(NDBT_Context *ctx, int loc, int rem, const char *msg) {
  ...
  while (1) {
    if (ctx->isTestStopped()) return false;
    int ret;
    if ((ret = ctx->getProperty(ackName, (Uint32)0)) != 0) break;
    if (++cnt % 100 == 0)
...
static bool poll_msg(NDBT_Context *ctx, int loc, int rem, char *msg) {
  char msgName[20], ackName[20];
...
static int recv_msg(NDBT_Context *ctx, int loc, int rem, char *msg) {
  uint cnt = 0;
  while (1) {
    if (ctx->isTestStopped()) return false;
    if (poll_msg(ctx, loc, rem, msg)) break;
    if (++cnt % 100 == 0)
...
static const NdbDictionary::Table *runBug48604createtable(NDBT_Context *ctx,
                                                          NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
...
static const NdbDictionary::Index *runBug48604createindex(NDBT_Context *ctx,
                                                          NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
...
int runBug48604(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const NdbDictionary::Table *pTab = 0;
  const NdbDictionary::Index *pInd = 0;
  ...
  do {
    ...
    while (result == NDBT_OK && loop++ < loops) {
      g_err << "loop:" << loop << endl;
      {
        ...
        const char *ops[][3] = {{"ozin", "oc", "oa"},  // 0: before 1-2: after
                                {"oziun", "oc", "oa"},
                                {"ozidn", "oc", "oa"},
                                {"ozicun", "oc", "oa"},
                                {"ozicuuun", "oc", "oa"},
...
int runBug48604ops(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const NdbDictionary::Table *pTab = 0;
  // const NdbDictionary::Index* pInd = 0;
        ...
        while (result == NDBT_OK && (c = *p++) != 0) {
          ...
          if (c == 'i' || c == 'u' || c == 'd') {
            ...
            for (i = 0; result == NDBT_OK && i < records; i++) {
              if (c == 'i') CHECK(ops.pkInsertRecord(pNdb, i, 1, opseq) == 0);
              if (c == 'u') CHECK(ops.pkUpdateRecord(pNdb, i, 1, opseq) == 0);
              if (c == 'd') CHECK(ops.pkDeleteRecord(pNdb, i, 1) == 0);
            }
            char op_str[2];
            sprintf(op_str, "%c", c);
            g_info << loc << ": op:" << op_str << " records:" << records
                   << endl;
            opseq++;
            continue;
          }
  ...
  if (result != NDBT_OK) ctx->stopTest();
  return result;
}

int runBug54651(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
...
static bool randomly(uint k, uint m) {
  uint n = urandom(m);
  return n < k;
}
...
struct ST_Obj {
  ...
  virtual bool is_trigger() const { return false; }
  virtual bool is_index() const { return false; }
  virtual bool is_table() const { return false; }
  virtual const char *realname() const { return name; }
...
static NdbOut &operator<<(NdbOut &out, const ST_Obj &obj) {
  out << obj.name << "[" << obj.id << "]";
  return out;
}
...
struct ST_Trg : public ST_Obj {
  ...
  bool is_trigger() const override { return true; }
  const char *realname() const override;
  ST_Trg(const char *a_db, const char *a_name) : ST_Obj(a_db, a_name) {
    ind = 0;
  }
  ~ST_Trg() override {}
};
...
struct ST_Ind : public ST_Obj {
  ...
  const ST_Trg &trg(int k) const { return *((*trglist)[k]); }
  ST_Trg &trg(int k) { return *((*trglist)[k]); }
  ST_Ind(const char *a_db, const char *a_name) : ST_Obj(a_db, a_name) {
    tab = 0;
    ind = 0;
    ind_r = 0;
...
const char *ST_Trg::realname() const {
...
struct ST_Tab : public ST_Obj {
  ...
  const ST_Ind &ind(int j) const { return *((*indlist)[j]); }
  ST_Ind &ind(int j) { return *((*indlist)[j]); }
  ST_Tab(const char *a_db, const char *a_name) : ST_Obj(a_db, a_name) {
    tab = 0;
    tab_r = 0;
...
void ST_Restarter::set_state(const ndb_mgm_node_state &new_state) {
  ...
  if (!first_time) require(old_state.node_type == new_state.node_type);
  old_state = new_state;
}

int ST_Restarter::get_status() {
  if (getStatus() == -1) return -1;
  int i;
  for (i = 0; i < (int)mgmNodes.size(); i++) set_state(mgmNodes[i]);
  for (i = 0; i < (int)ndbNodes.size(); i++) set_state(ndbNodes[i]);
  for (i = 0; i < (int)apiNodes.size(); i++) set_state(apiNodes[i]);
  first_time = false;
  return 0;
}
...
struct ST_Con {
  ...
  int node_id;
  int loop;
  const ST_Tab &tab(int i) const { return *((*tablist)[i]); }
  ST_Tab &tab(int i) { return *((*tablist)[i]); }
...
static int st_drop_all_tables(ST_Con &c) {
  g_info << "st_drop_all_tables" << endl;
  NdbDictionary::Dictionary::List list;
  chk2(c.dic->listObjects(list) == 0, c.dic->getNdbError());
  int n;
  for (n = 0; n < (int)list.count; n++) {
    const NdbDictionary::Dictionary::List::Element &element = list.elements[n];
...
static void st_init_objects(ST_Con &c, NDBT_Context *ctx) {
  ...
  for (i = 0; i < numTables; i++) {
    ...
    while (indspec != 0 && *indspec != 0) {
      ...
      int ind_len =
          snprintf(ind_name, ST_MAX_NAME_SIZE, "%sX%d", tab.name, tab.indcount);
      ...
      if (strcmp(type, "UNIQUE") == 0) {
...

        {
          ...
          int trg_len = snprintf(trg_name, ST_MAX_NAME_SIZE,
...
static int st_report_db_nodes(ST_Con &c, NdbOut &out) {
...
static int st_start_xcon(ST_Con &c) {
  require(c.xcon == 0);
...
static int st_stop_xcon(ST_Con &c) {
  require(c.xcon != 0);
...
struct ST_Errins {
  ...
  bool ends;              // end list
  ST_Errins() : value(0), code(0), master(-1), node(0), list(0), ends(true) {}
...
static NdbOut &operator<<(NdbOut &out, const ST_Errins &errins) {
...
static ST_Errins st_get_errins(ST_Con &c, const ST_Errins *list) {
  uint size = 0;
...
static int st_do_errins(ST_Con &c, ST_Errins &errins) {
  require(errins.value != 0);
  if (c.numdbnodes < 2)
    errins.master = 1;
...
// debug aid
#ifndef NDEBUG
static const ST_Obj *st_find_obj(const char *dbname, const char *name) {
  const ST_Obj *ret_objp = 0;
  int i;
  for (i = 0; i < (int)st_objlist.size(); i++) {
    const ST_Obj *objp = st_objlist[i];
    if (strcmp(objp->dbname, dbname) == 0 && strcmp(objp->name, name) == 0) {
      require(ret_objp == 0);
      ret_objp = objp;
    }
  }
...
static void st_set_commit_obj(ST_Con &c, ST_Obj &obj) {
  bool create_old = obj.create;
  bool commit_old = obj.commit;
  if (!c.tx_commit && !obj.commit) obj.create = !obj.create;
  obj.commit = true;
...
static void st_set_commit_ind(ST_Con &c, ST_Ind &ind) {
...
static void st_set_commit_tab(ST_Con &c, ST_Tab &tab) {
...
static void st_set_commit_all(ST_Con &c) {
  int i;
...
static void st_set_create_obj(ST_Con &c, ST_Obj &obj, bool create) {
  bool create_old = obj.create;
  bool commit_old = obj.commit;
  obj.create = create;
  obj.commit = !c.tx_on;
...
static void st_set_create_trg(ST_Con &c, ST_Trg &trg, bool create) {
  st_set_create_obj(c, trg, create);
}

static void st_set_create_ind(ST_Con &c, ST_Ind &ind, bool create) {
...
static void st_set_create_tab(ST_Con &c, ST_Tab &tab, bool create) {
  ...
  for (j = 0; j < tab.indcount; j++) {
    ...
    if (create == true)
      ...
      if (ind.exists()) st_set_create_ind(c, ind, false);
    }
  }
}
...
static bool st_known_type(
    const NdbDictionary::Dictionary::List::Element &element) {
  return element.id != ST_Obj::Skip;
}

static int st_find_object(const NdbDictionary::Dictionary::List &list,
                          NdbDictionary::Object::Type type, int id) {
  int n;
  for (n = 0; n < (int)list.count; n++) {
    const NdbDictionary::Dictionary::List::Element &element = list.elements[n];
    if (element.type == type && (int)element.id == id) return n;
  }
  return -1;
}

// filter out irrelevant by whatever means (we need listObjects2)
static int st_list_objects(ST_Con &c, NdbDictionary::Dictionary::List &list) {
  ...
  for (n = 0; n < (int)list.count; n++) {
    ...
    if (element.type == NdbDictionary::Object::UserTable) {
      ...
      for (i = 0; i < c.tabcount; i++) {
        ...
        if (strcmp(element.name, tab.name) == 0) keep[n]++;
      }
    }
    require(keep[n] <= 1);
  }
    ...
    if (element.type == NdbDictionary::Object::UniqueHashIndex ||
      ...
      for (i = 0; i < c.tabcount; i++) {
        ...
        for (j = 0; j < tab.indcount; j++) {
          ...
          if (strcmp(element.name, ind.name) == 0) keep[n]++;
        }
      }
    }
    require(keep[n] <= 1);
  ...
  for (n = 0; n < (int)list.count; n++) {
    ...
    if (element.type == NdbDictionary::Object::HashIndexTrigger) {
      int id, n2;
      chk2(sscanf(element.name, "NDB$INDEX_%d_UI", &id) == 1, element.name);
    ...
    if (element.type == NdbDictionary::Object::IndexTrigger) {
      int id, n2;
      chk2(sscanf(element.name, "NDB$INDEX_%d_CUSTOM", &id) == 1, element.name);
  ...
  // mark ignored
  for (n = 0; n < (int)list.count; n++) {
    NdbDictionary::Dictionary::List::Element &element = list.elements[n];
...
static bool st_match_obj(
    const ST_Obj &obj,
    const NdbDictionary::Dictionary::List::Element &element) {
  int veryverbose = 0;
  if (veryverbose) {
    g_info << "match:"
           << " " << obj.type << "-" << element.type << " " << obj.dbname << "-"
           << element.database << " " << obj.realname() << "-" << element.name
           << endl;
  }
...
st_verify_obj(const ST_Obj &obj, const NdbDictionary::Dictionary::List &list) {
  int found = 0;
  int n;
  for (n = 0; n < (int)list.count; n++) {
    const NdbDictionary::Dictionary::List::Element &element = list.elements[n];
    if (!st_known_type(element)) continue;
...
static int st_verify_list(ST_Con &c) {
  NdbDictionary::Dictionary::List list;
...
static int st_wait_idle(ST_Con &c) {
...
static int st_equal_column(const NdbDictionary::Column &c1,
...
static int st_equal_table(const NdbDictionary::Table &t1,
                          const NdbDictionary::Table &t2) {
...
static int st_equal_index(const NdbDictionary::Index &i1,
                          const NdbDictionary::Index &i2) {
...
static int st_verify_table(ST_Con &c, ST_Tab &tab) {
...
static int st_verify_index(ST_Con &c, ST_Ind &ind) {
  ST_Tab &tab = *ind.tab;
...
static int st_verify_all(ST_Con &c) {
...
static const uint ST_CommitFlag = 0;

static const uint ST_AbortFlag = NdbDictionary::Dictionary::SchemaTransAbort;

static const uint ST_BackgroundFlag =
    NdbDictionary::Dictionary::SchemaTransBackground;
...
static int st_begin_trans(ST_Con &c, int code = 0) {
  g_info << "begin trans";
  if (code == 0) {
    g_info << endl;
...
static int st_begin_trans(ST_Con &c, ST_Errins errins) {
...
static int st_begin_trans(ST_Con &c, ST_Retry retry) {
  int tries = 0;
...
static int st_end_trans(ST_Con &c, uint flags) {
...
static int st_end_trans_aborted(ST_Con &c, uint flags) {
  g_info << "end trans flags:" << hex << flags << endl;
  int res = c.dic->endSchemaTrans(flags);
  g_info << "end trans result:" << res << endl;
...
static int st_end_trans(ST_Con &c, ST_Errins errins, uint flags) {
  chk1(st_do_errins(c, errins) == 0);
...
static int st_end_trans_aborted(ST_Con &c, ST_Errins errins, uint flags) {
  chk1(st_do_errins(c, errins) == 0);
...
static int st_load_table(ST_Con &c, ST_Tab &tab, int rows = 1000) {
...
static int st_create_table(ST_Con &c, ST_Tab &tab, int code = 0) {
...
static int st_create_table(ST_Con &c, ST_Tab &tab, ST_Errins errins) {
  require(errins.code != 0);
  chk1(st_do_errins(c, errins) == 0);
...
static int st_drop_table(ST_Con &c, ST_Tab &tab, int code = 0) {
...
static int st_drop_table(ST_Con &c, ST_Tab &tab, ST_Errins errins) {
  require(errins.code != 0);
  chk1(st_do_errins(c, errins) == 0);
...
static int st_create_index(ST_Con &c, ST_Ind &ind, int code = 0) {
  ST_Tab &tab = *ind.tab;
  g_info << ind.name << ": create index on " << tab.name << "("
         << ind.colnames.c_str() << ")";
  if (code == 0) {
    g_info << endl;
    require(!ind.exists());
...
static int st_create_index(ST_Con &c, ST_Ind &ind, ST_Errins errins) {
  require(errins.code != 0);
  chk1(st_do_errins(c, errins) == 0);
...
static int st_drop_index(ST_Con &c, ST_Ind &ind, int code = 0) {
  ST_Tab &tab = *ind.tab;
...
static int st_drop_index(ST_Con &c, ST_Ind &ind, ST_Errins errins) {
  require(errins.code != 0);
  chk1(st_do_errins(c, errins) == 0);
...
static int st_create_table_index(ST_Con &c, ST_Tab &tab) {
...
static int st_drop_test_tables(ST_Con &c) {
  g_info << "st_drop_test_tables" << endl;
  int i;
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
    if (tab.exists()) chk1(st_drop_table(c, tab) == 0);
  }
  return 0;
err:
  return -1;
...
static const ST_Errins st_errins_begin_trans[] = {ST_Errins(6101, 780),
                                                  ST_Errins()};

static const ST_Errins st_errins_end_trans1[] = {
    ST_Errins(ERR_INSERT_MASTER_FAILURE1, 0, 1), ST_Errins()};

static const ST_Errins st_errins_end_trans2[] = {
    ST_Errins(ERR_INSERT_MASTER_FAILURE2, 0, 1), ST_Errins()};

static const ST_Errins st_errins_end_trans3[] = {
    ST_Errins(ERR_INSERT_MASTER_FAILURE3, 0, 1), ST_Errins()};

static const ST_Errins st_errins_table[] = {ST_Errins(6111, 783),
                                            ST_Errins(6121, 9121),
                                            // ST_Errins(6131, 9131),
                                            ST_Errins()};

static ST_Errins st_errins_index[] = {
    ST_Errins(st_errins_table), ST_Errins(6112, 783), ST_Errins(6113, 783),
    ST_Errins(6114, 783), ST_Errins(6122, 9122), ST_Errins(6123, 9123),
    ST_Errins(6124, 9124),
    // ST_Errins(6132, 9131),
    // ST_Errins(6133, 9131),
    // ST_Errins(6134, 9131),
...
static ST_Errins st_errins_index_create[] = {
    ST_Errins(st_errins_index), ST_Errins(6116, 783), ST_Errins(6126, 9126),
    // ST_Errins(6136, 9136),
    ST_Errins()};

static ST_Errins st_errins_index_drop[] = {ST_Errins(st_errins_index),
                                           ST_Errins()};

// specific test cases

static int st_test_create(ST_Con &c, int arg = -1) {
  int do_abort = (arg == 1);
  int i;
  ...
  chk1(st_verify_list(c) == 0);
  if (!do_abort) chk1(st_drop_test_tables(c) == 0);
  return NDBT_OK;
err:
  return NDBT_FAILED;
}

static int st_test_drop(ST_Con &c, int arg = -1) {
  int do_abort = (arg == 1);
  int i;
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
...
static int st_test_rollback_create_table(ST_Con &c, int arg = -1) {
  int i;
  chk1(st_begin_trans(c) == 0);
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
...
static int st_test_rollback_drop_table(ST_Con &c, int arg = -1) {
  int i;
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
    chk1(st_create_table(c, tab) == 0);
...
static int st_test_rollback_create_index(ST_Con &c, int arg = -1) {
  int i, j;
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
    if (tab.indcount < 1) continue;
    chk1(st_create_table(c, tab) == 0);
    chk1(st_begin_trans(c) == 0);
    for (j = 0; j < tab.indcount; j++) {
      ST_Ind &ind = tab.ind(j);
...
static int st_test_rollback_drop_index(ST_Con &c, int arg = -1) {
  int i, j;
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
    if (tab.indcount < 1) continue;
...
static int st_test_dup_create_table(ST_Con &c, int arg = -1) {
  int do_trans;
  int do_abort;
  int i;
  for (do_trans = 0; do_trans <= 1; do_trans++) {
    for (do_abort = 0; do_abort <= do_trans; do_abort++) {
      g_info << "trans:" << do_trans << " abort:" << do_abort << endl;
      for (i = 0; i < c.tabcount; i++) {
        ST_Tab &tab = c.tab(i);
        if (do_trans) chk1(st_begin_trans(c) == 0);
        chk1(st_create_table(c, tab) == 0);
...
static int st_test_dup_drop_table(ST_Con &c, int arg = -1) {
  int do_trans;
  int do_abort;
  int i;
  for (do_trans = 0; do_trans <= 1; do_trans++) {
...
static int st_test_dup_create_index(ST_Con &c, int arg = -1) {
  int do_trans;
  int do_abort;
  int i, j;
  for (do_trans = 0; do_trans <= 1; do_trans++) {
...
static int st_test_dup_drop_index(ST_Con &c, int arg = -1) {
  int do_trans;
  int do_abort;
  int i, j;
  for (do_trans = 0; do_trans <= 1; do_trans++) {
...
static int st_test_build_index(ST_Con &c, int arg = -1) {
  int i, j;
  for (i = 0; i < c.tabcount; i++) {
    ST_Tab &tab = c.tab(i);
    if (tab.indcount < 1) continue;
...
static ST_Errins st_test_local_create_list[] = {
    ...
    ST_Errins(8033, 293, 0),  ST_Errins(4003, 4237, 1),  // TUP trigger
    ST_Errins(4003, 4237, 0), ST_Errins(8034, 292, 1),   // TC index
    ST_Errins(8034, 292, 0)};

static int st_test_local_create(ST_Con &c, int arg = -1) {
  const int n = arg;
  ...
  for (i = 0; i < c.tabcount; i++) {
    ...
    for (j = 0; j < tab.indcount; j++) {
      ST_Ind &ind = tab.ind(j);
      if (only_unique && !ind.is_unique()) continue;
      if (!tabdone) {
        chk1(st_create_table(c, tab) == 0);
        chk1(st_load_table(c, tab) == 0);
        tabdone = true;
    ...
    if (tabdone) chk1(st_drop_table(c, tab) == 0);
  }
  return NDBT_OK;
err:
  return NDBT_FAILED;
...
static int st_test_trans(ST_Con &c, int arg = -1) {
...
static int st_test_create_table(ST_Con &c, int arg = -1) {
  ...
  for (i = 0; i < c.tabcount; i++) {
    ...
    if ((arg & ST_AllowErrins) && randomly(1, 3)) {
      ...
      if (simpletrans) {
        uint flags = 0;
        if ((arg & ST_AllowAbort) && randomly(4, 5)) flags |= ST_AbortFlag;
        chk1(st_end_trans(c, flags) == 0);
      }
    }
...
static int st_test_drop_table(ST_Con &c, int arg = -1) {
  bool trans = randomly(3, 4);
  bool simpletrans = !trans && randomly(1, 2);
  g_info << "trans:" << trans << " simpletrans:" << simpletrans << endl;
  if (trans) {
...
static int st_test_table(ST_Con &c, int arg = -1) {
...
static int st_test_create_index(ST_Con &c, int arg = -1) {
  bool trans = randomly(3, 4);
  bool simpletrans = !trans && randomly(1, 2);
  g_info << "trans:" << trans << " simpletrans:" << simpletrans << endl;
  if (trans) {
...
static int st_test_drop_index(ST_Con &c, int arg = -1) {
  bool trans = randomly(3, 4);
  bool simpletrans = !trans && randomly(1, 2);
  g_info << "trans:" << trans << " simpletrans:" << simpletrans << endl;
  if (trans) {
...
static int st_test_index(ST_Con &c, int arg = -1) {
...
static int st_test_anf_parse(ST_Con &c, int arg = -1) {
  int i;
...
static int st_test_anf_background(ST_Con &c, int arg = -1) {
  int i;
  chk1(st_start_xcon(c) == 0);
  {
    ST_Con &xc = *c.xcon;
...
static int st_test_anf_fail_begin(ST_Con &c, int arg = -1) {
  chk1(st_start_xcon(c) == 0);
  {
    ST_Con &xc = *c.xcon;
...
static int st_test_snf_parse(ST_Con &c, int arg = -1) {
  bool do_abort = (arg == 1);
  chk1(st_begin_trans(c) == 0);
  int node_id;
  node_id = -1;
...
static int st_test_mnf_parse(ST_Con &c, int arg = -1) {
  const NdbDictionary::Table *pTab;
  bool do_abort = (arg == 1);
  chk1(st_begin_trans(c) == 0);
  int node_id;
  ...
  for (i = 0; i < c.tabcount; i++) {
    ...
    // Verify that table is not in db
    c.dic->invalidateTable(tab.name);
    pTab = NDBT_Table::discoverTableFromDb(c.ndb, tab.name);
    chk1(pTab == NULL);
  }
  /*
    chk1(st_verify_all(c) == 0);
...
static int st_test_mnf_prepare(ST_Con &c, int arg = -1) {
  NdbRestarter restarter;
...
static int st_test_mnf_commit1(ST_Con &c, int arg = -1) {
  NdbRestarter restarter;
  // int master = restarter.getMasterNodeId();
...
static int st_test_mnf_commit2(ST_Con &c, int arg = -1) {
  NdbRestarter restarter;
  // int master = restarter.getMasterNodeId();
...
static int st_test_mnf_run_commit(ST_Con &c, int arg = -1) {
  const NdbDictionary::Table *pTab;
  NdbRestarter restarter;
  // int master = restarter.getMasterNodeId();
  int i;
  ...
  if (arg == FAIL_BEGIN) {
    ...
    if (restarter.getNumDbNodes() < 3) return NDBT_OK;
...
static int st_test_mnf_run_abort(ST_Con &c, int arg = -1) {
  NdbRestarter restarter;
  // int master = restarter.getMasterNodeId();
  const NdbDictionary::Table *pTab;
...
static int st_test_mnf_start_partial(ST_Con &c, int arg = -1) {
...
static int st_test_mnf_parse_partial(ST_Con &c, int arg = -1) {
...
static int st_test_mnf_flush_prepare_partial(ST_Con &c, int arg = -1) {
  ST_Errins errins(ERR_INSERT_PARTIAL_FLUSH_PREPARE_FAIL, 0,
                   1);  // slave skips flush prepare
  chk1(st_do_errins(c, errins) == 0);
  return st_test_mnf_run_commit(c, arg);
err:
  return -1;
...
static int st_test_mnf_prepare_partial(ST_Con &c, int arg = -1) {
  ST_Errins errins(ERR_INSERT_PARTIAL_PREPARE_FAIL, 0,
                   1);  // slave skips prepare
  chk1(st_do_errins(c, errins) == 0);
  return st_test_mnf_run_commit(c, arg);
err:
  return -1;
...
static int st_test_mnf_abort_parse_partial(ST_Con &c, int arg = -1) {
  ST_Errins errins(ERR_INSERT_PARTIAL_ABORT_PARSE_FAIL, 0,
                   1);  // slave skips abort parse
  chk1(st_do_errins(c, errins) == 0);
...
static int st_test_mnf_abort_prepare_partial(ST_Con &c, int arg = -1) {
  ST_Errins errins(ERR_INSERT_PARTIAL_ABORT_PREPARE_FAIL, 0,
                   1);  // slave skips abort prepare
  chk1(st_do_errins(c, errins) == 0);
  return st_test_mnf_run_abort(c, arg);
err:
  return -1;
...
static int st_test_mnf_flush_commit_partial(ST_Con &c, int arg = -1) {
  NdbRestarter restarter;
  ST_Errins errins(ERR_INSERT_PARTIAL_FLUSH_COMMIT_FAIL, 0,
                   1);  // slave skips flush commit
  chk1(st_do_errins(c, errins) == 0);
...
static int st_test_mnf_commit_partial(ST_Con &c, int arg = -1) {
...
static int st_test_mnf_flush_complete_partial(ST_Con &c, int arg = -1) {
  ST_Errins errins(ERR_INSERT_PARTIAL_FLUSH_COMPLETE_FAIL, 0,
                   1);  // slave skips flush complete
  chk1(st_do_errins(c, errins) == 0);
  return st_test_mnf_run_commit(c, arg);
err:
  return -1;
...
static int st_test_mnf_complete_partial(ST_Con &c, int arg = -1) {
  ST_Errins errins(ERR_INSERT_PARTIAL_COMPLETE_FAIL, 0,
                   1);  // slave skips complete
  chk1(st_do_errins(c, errins) == 0);
  return st_test_mnf_run_commit(c, arg);
err:
  return -1;
...
static int st_test_mnf_end_partial(ST_Con &c, int arg = -1) {
...
static int st_test_sr_parse(ST_Con &c, int arg = -1) {
  bool do_abort = (arg == 1);
  chk1(st_begin_trans(c) == 0);
  int i;
  for (i = 0; i < c.tabcount; i++) {
...
static NdbOut &operator<<(NdbOut &out, const ST_Test &test) {
  out << "CASE " << test.key;
  out << " " << test.name;
  if (test.arg != -1) out << "+" << test.arg;
  out << " - " << test.desc;
  return out;
}

static const ST_Test st_test_list[] = {
#define func(f) f, #f
    // specific ops
    {"a1", 1, 0, func(st_test_create), "create all within trans, commit"},
    {"a2", 1, 1, func(st_test_create), "create all within trans, abort"},
    {"a3", 1, 0, func(st_test_drop), "drop all within trans, commit"},
    {"a4", 1, 1, func(st_test_drop), "drop all within trans, abort"},
    {"b1", 1, -1, func(st_test_rollback_create_table),
     "partial rollback of create table ops"},
    {"b2", 1, -1, func(st_test_rollback_drop_table),
     "partial rollback of drop table ops"},
    {"b3", 1, -1, func(st_test_rollback_create_index),
     "partial rollback of create index ops"},
    {"b4", 1, -1, func(st_test_rollback_drop_index),
     "partial rollback of drop index ops"},
    {"c1", 1, -1, func(st_test_dup_create_table),
     "try to create same table twice"},
    {"c2", 1, -1, func(st_test_dup_drop_table), "try to drop same table twice"},
    {"c3", 1, -1, func(st_test_dup_create_index),
     "try to create same index twice"},
    {"c4", 1, -1, func(st_test_dup_drop_index), "try to drop same index twice"},
    {"d1", 1, -1, func(st_test_build_index), "build index on non-empty table"},
    {"e1", 1, 0, func(st_test_local_create),
     "fail trigger create in TC, master errins 8033"},
    {"e2", 2, 1, func(st_test_local_create),
     "fail trigger create in TC, slave errins 8033"},
    {"e3", 1, 2, func(st_test_local_create),
     "fail trigger create in TUP, master errins 4003"},
    {"e4", 2, 3, func(st_test_local_create),
     "fail trigger create in TUP, slave errins 4003"},
    {"e5", 1, 4, func(st_test_local_create),
     "fail index create in TC, master errins 8034"},
    {"e6", 2, 5, func(st_test_local_create),
     "fail index create in TC, slave errins 8034"},
    // random ops
    {"o1", 1, 0, func(st_test_trans), "start and stop schema trans"},
    {"o2", 1, ST_AllowAbort, func(st_test_trans),
     "start and stop schema trans, allow abort"},
    {"o3", 1, ST_AllowAbort | ST_AllowErrins, func(st_test_trans),
     "start and stop schema trans, allow abort errins"},
    //
    {"p1", 1, 0, func(st_test_create_table), "create tables at random"},
    {"p2", 1, ST_AllowAbort, func(st_test_create_table),
     "create tables at random, allow abort"},
    {"p3", 1, ST_AllowAbort | ST_AllowErrins, func(st_test_create_table),
     "create tables at random, allow abort errins"},
    //
    {"p4", 1, 0, func(st_test_table), "create and drop tables at random"},
    {"p5", 1, ST_AllowAbort, func(st_test_table),
     "create and drop tables at random, allow abort"},
    {"p6", 1, ST_AllowAbort | ST_AllowErrins, func(st_test_table),
     "create and drop tables at random, allow abort errins"},
    //
    {"q1", 1, 0, func(st_test_create_index), "create indexes at random"},
    {"q2", 1, ST_AllowAbort, func(st_test_create_index),
     "create indexes at random, allow abort"},
    {"q3", 1, ST_AllowAbort | ST_AllowErrins, func(st_test_create_index),
     "create indexes at random, allow abort errins"},
    //
    {"q4", 1, 0, func(st_test_index), "create and drop indexes at random"},
    {"q5", 1, ST_AllowAbort, func(st_test_index),
     "create and drop indexes at random, allow abort"},
    {"q6", 1, ST_AllowAbort | ST_AllowErrins, func(st_test_index),
     "create and drop indexes at random, allow abort errins"},
    // node failure and system restart
    {"u1", 1, -1, func(st_test_anf_parse), "api node fail in parse phase"},
    {"u2", 1, -1, func(st_test_anf_background),
     "api node fail after background trans"},
    {"u3", 2, -1, func(st_test_anf_fail_begin),
     "api node fail in middle of kernel begin trans"},
    //
    {"v1", 2, 0, func(st_test_snf_parse),
     "slave node fail in parse phase, commit"},
    {"v2", 2, 1, func(st_test_snf_parse),
     "slave node fail in parse phase, abort"},
    {"w1", 1, 0, func(st_test_sr_parse),
     "system restart in parse phase, commit"},
    {"w2", 1, 1, func(st_test_sr_parse),
     "system restart in parse phase, abort"},
#ifdef ndb_master_failure
    {"x1", 2, 0, func(st_test_mnf_parse),
     "master node fail in parse phase, commit"},
    {"x2", 2, 1, func(st_test_mnf_parse),
     "master node fail in parse phase, abort"},
    {"x3", 2, 0, func(st_test_mnf_prepare),
     "master node fail in prepare phase"},
    {"x4", 2, 0, func(st_test_mnf_commit1),
     "master node fail in start of commit phase"},
    {"x5", 2, 0, func(st_test_mnf_commit2),
     "master node fail in end of commit phase"},
    {"y1", 2, SUCCEED_COMMIT, func(st_test_mnf_start_partial),
     "master node fail in start phase, retry will succeed"},
    {"y2", 2, FAIL_CREATE, func(st_test_mnf_parse_partial),
     "master node fail in parse phase, partial rollback"},
    {"y3", 2, FAIL_END, func(st_test_mnf_flush_prepare_partial),
     "master node fail in flush prepare phase, partial rollback"},
    {"y4", 2, FAIL_END, func(st_test_mnf_prepare_partial),
     "master node fail in prepare phase, partial rollback"},
    {"y5", 2, SUCCEED_COMMIT, func(st_test_mnf_flush_commit_partial),
     "master node fail in flush commit phase, partial rollback"},
    {"y6", 2, SUCCEED_COMMIT, func(st_test_mnf_commit_partial),
     "master node fail in commit phase, commit, partial rollforward"},
    {"y7", 2, SUCCEED_COMMIT, func(st_test_mnf_flush_complete_partial),
     "master node fail in flush complete phase, commit, partial rollforward"},
    {"y8", 2, SUCCEED_COMMIT, func(st_test_mnf_complete_partial),
     "master node fail in complete phase, commit, partial rollforward"},
    {"y9", 2, SUCCEED_COMMIT, func(st_test_mnf_end_partial),
     "master node fail in end phase, commit, partial rollforward"},
    {"z1", 2, SUCCEED_ABORT, func(st_test_mnf_abort_parse_partial),
     "master node fail in abort parse phase, partial rollback"},
    {"z2", 2, FAIL_END, func(st_test_mnf_abort_prepare_partial),
     "master node fail in abort prepare phase, partial rollback"},
    {"z3", 2, 1, func(st_test_mnf_prepare),
     "master node fail in prepare phase in background"},
    {"z4", 2, 1, func(st_test_mnf_commit1),
     "master node fail in start of commit phase in background"},
    {"z5", 2, 1, func(st_test_mnf_commit2),
...
static const int st_test_count = sizeof(st_test_list) / sizeof(st_test_list[0]);

static const char *st_test_case = 0;
static const char *st_test_skip = 0;

static bool st_test_match(const ST_Test &test) {
  const char *p = 0;
  if (st_test_case == 0) goto skip;
  if (strstr(st_test_case, test.key) != 0) goto skip;
  p = strchr(st_test_case, test.key[0]);
  if (p != 0 && (p[1] < '0' || p[1] > '9')) goto skip;
  return false;
skip:
  if (st_test_skip == 0) return true;
  if (strstr(st_test_skip, test.key) != 0) return false;
  p = strchr(st_test_skip, test.key[0]);
  if (p != 0 && (p[1] < '0' || p[1] > '9')) return false;
  return true;
}

static int st_test(ST_Con &c, const ST_Test &test) {
  chk1(st_end_trans(c, ST_AbortFlag) == 0);
  chk1(st_drop_test_tables(c) == 0);
...
int runSchemaTrans(NDBT_Context *ctx, NDBT_Step *step) {
#ifdef NDB_USE_GET_ENV
  {
  ...
  }
  {
    ...
    if (env != 0) st_random_seed = atoi(env);
  }
#endif
  ...
  if (st_random_seed == -1) st_random_seed = NdbHost_GetProcessId();
  if (st_random_seed != 0) {
  ...
  for (c.loop = 0; numloops == 0 || c.loop < numloops; c.loop++) {
    ...
    if (st_random_seed == 0) ndb_srand(c.loop);
    int i;
    for (i = 0; i < st_test_count; i++) {
      const ST_Test &test = st_test_list[i];
...
int runFailCreateHashmap(NDBT_Context *ctx, NDBT_Step *step) {
...
int runCreateHashmaps(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  // int nodeId = restarter.getMasterNodeId();
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
...
int runFailAddPartition(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  altered.setFragmentCount(org->getFragmentCount() + restarter.getNumDbNodes());
  ...
  for (int l = 0; l < loops; l++) {
    for (unsigned i0 = 0; lst[i0]; i0++) {
      ...
      CHECK2(res != 0, "failed to fail after error insert " << errval);
      CHECK2(restarter.insertErrorInNode(nodeId, 0) == 0,
             "failed to clear error insert");
      CHECK(restarter.dumpStateAllNodes(&dump2, 1) == 0);
      NdbSleep_MilliSleep(SAFETY);  // Hope that snapshot has arrived
...
int runTableAddPartition(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  ...
  for (int l = 0; l < loops && result == NDBT_OK; l++) {
    ...
    CHECK2(dict->alterTable(*oldTable, newTable) == 0, "TableAddAttrs failed");
...
int runBug41905(NDBT_Context *ctx, NDBT_Step *step) {
  const NdbDictionary::Table *pTab = ctx->getTab();
  ...
  while (ret == NDBT_OK) {
    ...
    if (old_cols != cols || old_vers != vers) {
      g_err << __LINE__ << ": "
            << "got cols,vers " << old_cols << "," << old_vers << " expected "
            << cols << "," << vers << endl;
      ret = NDBT_FAILED;
      break;
    }
    if (old_cols >= 100) break;
    ...
    NDBT_Attribute newCol(colName, NdbDictionary::Column::Unsigned, 1, false,
                          true, (CHARSET_INFO *)0,
                          NdbDictionary::Column::StorageTypeMemory, true);
    ...
    if (removeEarly) pDic->removeTableGlobal(*pOldTab, 0);
...
int runBug41905getTable(NDBT_Context *ctx, NDBT_Step *step) {
  const NdbDictionary::Table *pTab = ctx->getTab();
  BaseString tabName(pTab->getName());
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  ...
  while (1) {
    while (1) {
      if (ctx->getProperty("Bug41905") == 1) break;
      if (ctx->getProperty("Bug41905") == 3) goto out;
      NdbSleep_MilliSleep(10);
    }
...
static int createIndexes(NdbDictionary::Dictionary *pDic,
...
int runBug46552(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  ...
  if (res.getNumDbNodes() < 2 || num_replicas != 2) return NDBT_OK;
  ...
  if (createIndexes(pDic, tab1, 4)) return NDBT_FAILED;
  ...
  if (createIndexes(pDic, tab0, 4)) return NDBT_FAILED;
...
int runBug46585(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  NdbDictionary::Table tab(*ctx->getTab());
  NdbRestarter res;
  ...
  for (int i = 0; i < loops; i++) {
    const NdbDictionary::Table *org = pDic->getTable(tab.getName());
    {
      ...
      CHECK2(trans.loadTable(pNdb, records) == 0, "load table failed");
    }
    ...
    altered.setPartitionBalance(
        NdbDictionary::Object::PartitionBalance_Specific);
    ndbout_c("alter from %u to %u partitions", org->getFragmentCount(),
    ...
    CHECK2(result == 0, "failed to alter");
    ...
    pDic->invalidateTable(tab.getName());
    {
      ...
      CHECK2(trans.scanUpdateRecords(pNdb, records) == 0, "scan update failed");
    ...
    switch (i % 2) {
      case 0:
        if (res.getNumDbNodes() > 1) {
          ...
          CHECK2(res.restartOneDbNode(nodeId, false, true, true) == 0,
                 "restart one node failed");
          CHECK2(res.waitNodesNoStart(&nodeId, 1) == 0,
                 "wait node started failed");
          CHECK2(res.startNodes(&nodeId, 1) == 0, "start node failed");
          break;
        }
    ...
    CHECK2(res.waitClusterStarted() == 0, "wait cluster started failed");
    CHK_NDB_READY(pNdb);

    Uint32 restartGCI = 0;
    CHECK2(pDic->getRestartGCI(&restartGCI) == 0, "getRestartGCI failed");
    ...
    pDic->invalidateTable(tab.getName());
    {
      ...
      int cnt;
      CHECK2(trans.selectCount(pNdb, 0, &cnt) == 0, "select count failed");

      CHECK2(cnt == records, "table does not have correct record count: "
                                 << cnt << " != " << records);
      ...
      CHECK2(trans.clearTable(pNdb) == 0, "clear table failed");
    }
  }
...
int runBug53944(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  NdbDictionary::Table tab(*ctx->getTab());
  NdbRestarter res;
  ...
  ndbout_c("reused %u table-ids out of %u", (unsigned)reused,
           (unsigned)ids.size());
...
static void sync_main_step(NDBT_Context *ctx, NDBT_Step *step,
                           const char *state) {
  // total sub-steps
...
static void sync_sub_step(NDBT_Context *ctx, NDBT_Step *step,
                          const char *state) {
...
static int runBug58277createtable(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
...
static int runBug58277loadtable(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  const int rows = ctx->getNumRecords();
  ...
  do {
    ...
    retry:
      ...
      do {
        ...
        if (res != 0 && pTx->getNdbError().status == NdbError::TemporaryError) {
          retries--;
          if (retries >= 0) {
            pTx->close();
            NdbSleep_MilliSleep(10);
...
static int runBug58277createindex(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  const char *tabname = tabName_Bug58277;
...
int runBug58277errtest(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const int loops = ctx->getNumLoops();
  int result = NDBT_OK;
  ...
  while (!ctx->isTestStopped() && errloop < errloops) {
    ...
    for (int i = 0; i < errcnt; i += 2) {
      ...
      CHK2((pSOp = pSTx->getNdbIndexScanOperation(pInd)) != 0,
           pSTx->getNdbError());
      ...
      CHK2(pSOp->getNdbError().code == ec,
           "expect " << ec << " got " << pSOp->getNdbError());
...
int runBug58277drop(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  const char *tabname = tabName_Bug58277;
...
static int runBug58277scanop(NDBT_Context *ctx, NDBT_Step *step,
                             int cnt[1 + 3]) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  ...
  do {
    const NdbDictionary::Index *pInd = 0;
    {
      ...
      pDic->invalidateIndex(indname, tabname);
      CHK3((pInd = pDic->getIndex(indname, tabname)) != 0, pDic->getNdbError(),
           x);
    }
    ...
    for (int i = 0; i < range_cnt;) {
      ...
      int thi = -1;
      if (rand() % 5 == 0) {
        if (rand() % 5 != 0) tlo = 0 + rand() % 2;
        if (rand() % 5 != 0) thi = 2 + rand() % 2;
      } else
        tlo = 4;
      // apparently no bounds is not allowed (see also bug#57396)
      if (tlo == -1 && thi == -1) continue;
      Uint32 blo = 0;
      Uint32 bhi = 0;
      if (tlo != -1) {
        blo = rand() % rows;
    ...
    while (1) {
      ...
      while (1) {
        ...
        if (type == 2)  // insert->update
          type = 1;
        if (scan_delete) type = 3;
        do {
          if (type == 1) {
            NdbOperation *pOp = 0;
            CHK2((pOp = pSOp->updateCurrentTuple(pTx)) != 0,
                 pSOp->getNdbError());
...
int runBug58277scan(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
...
static int runBug58277pkop(NDBT_Context *ctx, NDBT_Step *step, int cnt[1 + 3]) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  const int rows = ctx->getNumRecords();
...
int runBug58277pk(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
...
int runBug58277rand(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  NdbRestarter restarter;
...
int runBug58277(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const int loops = ctx->getNumLoops();
  int result = NDBT_OK;
...
int runBug57057(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const int loops = ctx->getNumLoops();
  int result = NDBT_OK;
 ...
 */
static int runGetTabInfoRef(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  if (restarter.getNumDbNodes() == 1) {
    g_info << "Cannot do this test with just one datanode." << endl;
    return NDBT_OK;
  ...
  if (restarter.restartNodes(
          nodeSet.getBase(), (int)nodeSet.size(),
          NdbRestarter::NRRF_NOSTART | NdbRestarter::NRRF_ABORT)) {
...
int runBug13416603(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  NdbIndexStat is;
  NdbRestarter res;
...

  {
    ...
    if (partitions.size() == 1) goto cleanup;
    ...
    for (unsigned i = 0; i < partitions.size(); i++) {
      ...
      res.restartNodes(partitions[i].getBase(), partitions[i].size(),
                       NdbRestarter::NRRF_NOSTART | NdbRestarter::NRRF_ABORT);
      res.waitNodesNoStart(partitions[i].getBase(), partitions[i].size());
      ...
      res.startNodes(partitions[i].getBase(), partitions[i].size());
      res.waitClusterStarted();
      CHK_NDB_READY(pNdb);
    }
  }
...
cleanup:
  ...
  {
    const int restartState[] = {DumpStateOrd::CmvmiSetRestartOnErrorInsert,
                                NRT_DoStart_Restart};

    require(res.dumpStateAllNodes(
                restartState, sizeof restartState / sizeof restartState[0]) ==
            0);
  }
...
int runIndexStatCreate(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbIndexStat is;
...
int runWL946(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const int loops = ctx->getNumLoops();
...
#ifdef NDB_USE_GET_ENV
  {
    ...
    if (p != 0 && strchr("1Y", p[0]) != 0) keep_table = true;
  }
#endif
#endif
  int result = NDBT_OK;
  ...
  for (int loop = 0; loop < loops; loop++) {
    ...
    for (int i = 0; i < Colcnt; i++) {
      ...
      if (d.flag & 8) {
        ...
        CHK2((px = pDic->getIndex(d.indname, tabname)) != 0,
             pDic->getNdbError());
        pInd[i] = px;
      }
    }
    ...
    for (int j = 0; j < scanloops; j++) {
      ...
      for (int i = 0; i < Colcnt; i++) {
        ...
        if (d.flag & 8) {
          ...
          CHK2(trans.scanReadRecords(pNdb, px, records) == 0,
               trans.getNdbError());
        }
      }
...
int getOrCreateDefaultHashMap(NdbDictionary::Dictionary &dict,
                              NdbDictionary::HashMap &hm, Uint32 buckets,
                              Uint32 fragments) {
...
int Bug14645319_createTable(Ndb *pNdb, NdbDictionary::Table &tab, int when,
  ...
  Bug14645319_createTable_args &args =
      *static_cast<Bug14645319_createTable_args *>(arg);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  if (when == 0) {
...
int runBug14645319(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int failures = 0;
  ...
  test_case test_cases[] = {
      {"Simulate online reorg, may or may not change hashmap depending on "
       "default fragment count",
       3, 120, 0, NDB_DEFAULT_HASHMAP_BUCKETS, 0},
      {"Keep old hashmap since no new fragments", 3, 120, 3,
       NDB_DEFAULT_HASHMAP_BUCKETS, 120},
      {"Keep old hashmap size since old size a multiple of new fragment count",
       3, 120, 6, NDB_DEFAULT_HASHMAP_BUCKETS, 120},
      {"Keep old hashmap size since new size not a multiple of old", 3, 130, 6,
       NDB_DEFAULT_HASHMAP_BUCKETS, 130},
      {"Extend hashmap", 3, 120, 7, NDB_DEFAULT_HASHMAP_BUCKETS,
       NDB_DEFAULT_HASHMAP_BUCKETS},
      {"Keep old hashmap size since old size not multiple of old fragment "
       "count",
       5, 84, 7, 42, 84},
      {"Shrink hashmap", 3, 120, 6, 60, 60},
  };
  ...
  for (size_t testi = 0; testi < NDB_ARRAY_SIZE(test_cases); testi++) {
    ...
    do {
      ...
      result = NDBT_Tables::createTable(pNdb, args.template_name, false, false,
                                        Bug14645319_createTable, &args);
      if (result != 0) break;
    ...
    if (result == 0) {
      ndbout << "Test#" << (testi + 1) << " '" << test_cases[testi].description
             << "' passed"
             << " (" << old_buckets << " => " << test_cases[testi].new_buckets
             << " => " << test_cases[testi].expected_buckets << ")" << endl;
    } else {
      ndbout << "Test#" << (testi + 1) << " '" << test_cases[testi].description
             << "' failed"
             << " (" << old_buckets << " => " << test_cases[testi].new_buckets
             << " => " << new_buckets
             << " expected: " << test_cases[testi].expected_buckets << ")"
             << endl;
      failures++;
    }
  }
...
static int fk_compare_icol(const void *p1, const void *p2) {
  const Fkdef::Col &col1 = *(const Fkdef::Col *)p1;
  const Fkdef::Col &col2 = *(const Fkdef::Col *)p2;
  return col1.icol - col2.icol;
}

static int fk_type(int t) {
  if (t == NdbDictionary::Object::ForeignKey ||
...
static int fk_compare_element(const void *p1, const void *p2) {
  ...
  int k = 0;
  if ((k = fk_type(e1.type) - fk_type(e2.type)) != 0) return k;
  if ((k = e1.type - e2.type) != 0) return k;
  if ((k = (int)e1.id - (int)e2.id) != 0) return k;
  return 0;
}

static bool fk_find_element(const Fkdef::List &list, int type,
                            const char *database, const char *name) {
  int found = 0;
  for (int i = 0; i < (int)list.list->count; i++) {
    const NdbDictionary::Dictionary::List::Element &e = list.list->elements[i];
    if (e.type == type && strcmp(e.database, database) == 0 &&
...
static void fk_define_tables1(Fkdef &d) {
  d.ntab = 2;
...
static void fk_define_keys1(Fkdef &d) {
  d.nkey = 1;
...
static void fk_define_tables2(Fkdef &d) {
...
static void fk_define_keys2(Fkdef &d) {
  ...
  while (k < nkey && ntry++ < ntrymax) {
    ...
    if (!di0.unique || di0.ncol != di1.ncol) continue;
    if (i0 == i1 && k0 == k1)
...
static void fk_define_tables(Fkdef &d) {
  if (d.testcase == 1)
    fk_define_tables1(d);
...
static void fk_define_keys(Fkdef &d) {
  if (d.nokeys) {
    d.nkey = 0;
    return;
  }
...
static void fk_undefine_keys(Fkdef &d) { d.nkey = 0; }

static void fk_define_all(Fkdef &d) {
  fk_define_tables(d);
  fk_define_keys(d);
}

static int fk_create_table(Fkdef &d, Ndb *pNdb, int i) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    Fkdef::Tab &dt = d.tab[i];
    ...
    for (int k = 1; k < dt.nind; k++)  // skip pk
    {
      ...
      CHK2(pDic->createIndex(ind) == 0, pDic->getNdbError());
      const NdbDictionary::Index *pInd = 0;
      CHK2((pInd = pDic->getIndex(di.indname, dt.tabname)) != 0,
           pDic->getNdbError());
...
static int fk_create_tables(Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
  for (int i = 0; i < d.ntab; i++) {
...
static int fk_create_key(Fkdef &d, Ndb *pNdb, int k) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    Fkdef::Key &dk = d.key[k];
...
static int fk_create_keys(Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
...
static int fk_alter_table(Fkdef &d, Ndb *pNdb, int i) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    Fkdef::Tab &dt = d.tab[i];
...
static int fk_alter_tables(Fkdef &d, Ndb *pNdb, bool atrandom) {
  int result = NDBT_OK;
  for (int i = 0; i < d.ntab; i++) {
...
static int fk_create_all(Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
  do {
...
static int fk_verify_table(const Fkdef &d, Ndb *pNdb, int i) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
...
static int fk_verify_tables(const Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
  for (int i = 0; i < d.ntab; i++) {
...
static int fk_verify_key(const Fkdef &d, Ndb *pNdb, int k) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    ...
    CHK2(strcmp(dk.fullname, key.getName()) == 0,
         dk.fullname << " != " << key.getName());
...
static int fk_verify_keys(const Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
  for (int k = 0; k < d.nkey; k++) {
...
static int fk_verify_ddl(const Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
  do {
...
static int fk_verify_dml(const Fkdef &d, Ndb *pNdb, int records) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  const int batch = 1;
  ...
  do {
    if (!(d.testcase == 1 && records > 0)) break;
    ...
    CHK2(tx0.loadTable(pNdb, records, batch, allowCV) == 0, tx0.getNdbError());
    // insert into child t1 - ok (b1 is 0, a0 is 0,1,2,..)
    CHK2(tx1.loadTable(pNdb, records, batch, allowCV) == 0, tx1.getNdbError());
...
static int fk_retrieve_list(Fkdef &d, Ndb *pNdb, Fkdef::List &list) {
  (void)d;
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    ...
    list.keystart = 0;
    for (int i = 0; i < (int)list.list->count; i++) {
      NdbDictionary::Dictionary::List::Element &e = list.list->elements[i];
      if (e.database == 0) {
        e.database = new char[1];
        e.database[0] = 0;
      }
      if (!fk_type(e.type)) list.keystart++;
      g_info << "ob " << i << ":"
             << " type=" << e.type << " id=" << e.id << " db=" << e.database
             << " name=" << e.name << endl;
      if (i > 0) {
        const NdbDictionary::Dictionary::List::Element &e2 =
    ...
    g_info << "list count=" << list.list->count << " keystart=" << list.keystart
           << endl;
  } while (0);
  return result;
}

static int fk_verify_list(Fkdef &d, Ndb *pNdb, bool ignore_keys) {
  int result = NDBT_OK;
  do {
    ...
    if (list1.list == 0) {
      ...
      for (int i = 0; i < count1; i++) {
        ...
        CHK2(e1.type == e2.type, i << ": " << e1.type << " != " << e2.type);
        CHK2(e1.id == e2.id, i << ": " << e1.id << " != " << e2.id);
    ...
    for (int i = 0; i < d.ntab; i++) {
      const Fkdef::Tab &dt = d.tab[i];
      CHK2(fk_find_element(list1, NdbDictionary::Object::UserTable, "TEST_DB",
                           dt.tabname),
           dt.tabname);
...
static int fk_drop_table(Fkdef &d, Ndb *pNdb, int i, bool force) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    Fkdef::Tab &dt = d.tab[i];
    g_info << "drop table " << dt.tabname << (force ? " (force)" : "") << endl;
...
static int fk_drop_tables(Fkdef &d, Ndb *pNdb, bool force) {
  int result = NDBT_OK;
  for (int i = 0; i < d.ntab; i++) {
...
static int fk_drop_key(Fkdef &d, Ndb *pNdb, int k, bool force) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    Fkdef::Key &dk = d.key[k];
    g_info << "drop key " << dk.fullname << (force ? " (force)" : "") << endl;
    NdbDictionary::ForeignKey key;
...
static int fk_drop_keys(Fkdef &d, Ndb *pNdb, bool force) {
  int result = NDBT_OK;
  for (int k = 0; k < d.nkey; k++) {
...
static int fk_drop_all(Fkdef &d, Ndb *pNdb, bool force) {
  int result = NDBT_OK;
  do {
...
static void fk_dropped_all(Fkdef &d) {
  for (int i = 0; i < d.ntab; i++) {
    Fkdef::Tab &dt = d.tab[i];
    dt.retrieved = false;
    for (int k = 0; k < dt.nind; k++) {
...
static int fk_create_all_random(Fkdef &d, Ndb *pNdb) {
  int result = NDBT_OK;
  int ntab = 0;
  int nkey = 0;
  do {
    ...
    for (int k = 0; k < d.nkey; k++) {
      Fkdef::Key &dk = d.key[k];
      if (!dk.retrieved && myRandom48(3) == 0 && dk.tab0->retrieved &&
          dk.tab1->retrieved) {
...
static int fk_drop_indexes_under(const Fkdef &d, Ndb *pNdb) {
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  int result = NDBT_OK;
  do {
    for (int i = 0; i < d.ntab; i++) {
...
// for manual testing
static void fk_env_options(Fkdef &d) {
  // random seed
...
#ifdef NDB_USE_GET_ENV
  {
    ...
    if (p != 0 && strchr("1Y", p[0]) != 0) d.nokeys = true;
  }
#endif
...
#ifdef NDB_USE_GET_ENV
  {
    ...
    if (p != 0 && strchr("1Y", p[0]) != 0) d.nodrop = true;
  }
#endif
}

int runFK_SRNR(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const int loops = ctx->getNumLoops();
  const int records = ctx->getNumRecords();
  int result = NDBT_OK;
  ...
  if (result != NDBT_OK) {
    if (!d.nodrop) (void)fk_drop_all(d, pNdb, true);
  }
  return result;
}

int runFK_TRANS(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  const int loops = ctx->getNumLoops();
  const int records = ctx->getNumRecords();
...
int runFK_Bug18069680(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const int loops = ctx->getNumLoops();
  const int records = ctx->getNumRecords();
  int result = NDBT_OK;
...
int runDictTO_1(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDic = pNdb->getDictionary();
  NdbRestarter restarter;
  ...
  for (int i = 0; i < ctx->getNumLoops(); i++) {
    ...
    for (int j = 0; j < (i + 1); j++) {
      pDic->dropTable(
          BaseString(ctx->getTab()->getName()).appfmt("_EXTRA_%u", j).c_str());
    }

    {
      g_info << "check all resource usage" << endl;
      for (int j = 0; j < restarter.getNumDbNodes(); j++) {
        if (restarter.getDbNodeId(j) == master) continue;
...
int runIndexStatTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
  const NdbDictionary::Table *pTab = ctx->getTab();
  char idxname[20];
  ...
  if (errorCode != 4008) {
    ndbout << "Error: updateIndexStat failed with wrong error " << errorCode
           << endl;
    return NDBT_FAILED;
  }
...
int runForceGCPWait(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
  NdbRestarter restarter;
  ...
  int errorCode = pDict->getNdbError().code;
  if (errorCode != 4008) {
    ndbout << "Error: forceGCPWait failed with wrong error " << errorCode
           << endl;
    return NDBT_FAILED;
  }
 ...
 */
static int changeStartDiskPageBufMem(NDBT_Context *ctx, NDBT_Step *step) {
  Config conf;
  NdbRestarter restarter;
  Uint64 new_diskpage_buffer = 4 * 1024 * 1024;  // Configured minimum value
  Uint64 start_disk_page_buffer =
  ...
  g_err << "Restarting nodes to apply config change from " << saved_old_value
        << " to " << start_disk_page_buffer << endl;
 ...
 */
int runCreateLogFileGroupTableSpace(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
 ...
 */
int runDropTableSpaceLG(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);

  // Read the info about the test created the table space
  // "DEFAULT-TS" from the test case context.
  Uint32 ts_created = ctx->getProperty("TABLESPACECREATED", (Uint32)0);
 ...
 *      fails otherwise.
 */
int runCreateManyDataFiles(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
  int result = NDBT_FAILED;

  // Add many data files until disk page buffer gets filled
  Uint32 data_files_to_create = ctx->getProperty("DATAFILES", (Uint32)200);
  NdbDictionary::Datafile df;
  uint created_files = 0;  // How many files are created so far
  char datafilename[256];

  for (Uint32 datafile = 0; datafile < data_files_to_create; ++datafile) {
    BaseString::snprintf(datafilename, sizeof(datafilename), "datafile%d",
                         datafile);
    ...
    if (res != 0) {
      ...
      if (error == 1517) {
        ...
        g_err << "Failed to create datafile " << datafilename << endl
              << pDict->getNdbError() << endl;
      }
      break;
    }
...
int runManyNdbObjectsGetTable(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  /*
   * The below created Ndb objects will most likely get consecutive
   * blocknumbers. In that case eigth of them (0x12, 0x52, 0x92, 0xd2, 0x111,
   * 0x151, 0x191, 0x1d1) will if treated as data node block numbers be mapped
   * to block number of V_QUERY. By provoking a GET_TABINFOREQ to be sent from
   * each Ndb object will demonstrate Bug#35154637.
   */
...
TESTCASE("DropTableConcurrentLCP", "Drop a table while LCP is ongoing\n") {
...
TESTCASE("FragmentTypeSingle", "Create the table with fragment type Single\n") {
...
TESTCASE("TableRename", "Test basic table rename") {
  INITIALIZER(runTableRename);
}
TESTCASE("TableRenameSR", "Test that table rename can handle system restart") {
  INITIALIZER(runTableRenameSR);
}
TESTCASE("ColumnRename", "Test basic column rename") {
...
TESTCASE("DictionaryPerf", "") { INITIALIZER(runTestDictionaryPerf); }
TESTCASE("CreateLogfileGroup", "") { INITIALIZER(runCreateLogfileGroup); }
...
TESTCASE("CreateDiskTable", "") { INITIALIZER(runCreateDiskTable); }
TESTCASE("FailAddFragment",
         "Fail add fragment or attribute in ACC or TUP or TUX\n") {
  INITIALIZER(runFailAddFragment);
}
TESTCASE("Restart_NF1", "DICT ops during node graceful shutdown (not master)") {
  TC_PROPERTY("Restart_NF_ops", 1);
  TC_PROPERTY("Restart_NF_type", 1);
  STEP(runRestarts);
  STEP(runDictOps);
}
TESTCASE("Restart_NF2", "DICT ops during node shutdown abort (not master)") {
  TC_PROPERTY("Restart_NF_ops", 1);
  TC_PROPERTY("Restart_NF_type", 2);
  STEP(runRestarts);
  STEP(runDictOps);
}
TESTCASE("Restart_NR1", "DICT ops during node startup (not master)") {
...
TESTCASE("Bug29186", "") { INITIALIZER(runBug29186); }
TESTCASE("Bug48604",
...
TESTCASE("Bug54651", "") { INITIALIZER(runBug54651); }
/** telco-6.4 **/
TESTCASE("SchemaTrans", "Schema transactions") {
  ALL_TABLES();
  STEP(runSchemaTrans);
}
TESTCASE("FailCreateHashmap", "Fail create hashmap") {
  INITIALIZER(runFailCreateHashmap);
}
TESTCASE("FailAddPartition", "Fail add partition") {
...
TESTCASE("Bug46552", "") { INITIALIZER(runBug46552); }
TESTCASE("Bug46585", "") {
  INITIALIZER(runWaitStarted);
  INITIALIZER(runBug46585);
}
TESTCASE("Bug53944", "") { INITIALIZER(runBug53944); }
TESTCASE("Bug58277",
...
TESTCASE("GetTabInfoRef",
         "Regression test for bug #14647210 'CAN CRASH ALL "
...
TESTCASE("IndexStatCreate", "") { STEPS(runIndexStatCreate, 10); }
TESTCASE("WL946",
         "Time types with fractional seconds.\n"
         "Give any tablename as argument (T1)") {
  INITIALIZER(runWL946);
}
TESTCASE("Bug14645319", "") { STEP(runBug14645319); }
TESTCASE("FK_SRNR1",
...
TESTCASE("CreateHashmaps", "Create (default) hashmaps") {
  INITIALIZER(runCreateHashmaps);
}
TESTCASE("DictTakeOver_1", "") { INITIALIZER(runDictTO_1); }
...
TESTCASE("CreateManyDataFiles",
         "Test lack of DiskPageBufferMemory "
...
TESTCASE("TableAddAttrsUpdateMaxRecSzForLCP",
         "Adding attributes to an existing"
