...
int NdbBackup::clearOldBackups() {
  ...
  for (size_t i = 0; i < ndbNodes.size(); i++) {
    ...
    const char *host;
    if (!getHostName(nodeId, &host)) return -1;
...
int NdbBackup::start(unsigned int &_backup_id, int flags,
                     unsigned int user_backup_id, unsigned int logtype,
...
loop:
  if (ndb_mgm_start_backup4(handle, flags, &_backup_id, &reply, user_backup_id,
                            logtype, encryption_password,
                            password_length) == -1) {
...
int NdbBackup::checkBackupStatus() {
...
std::string NdbBackup::getBackupDataDirForNode(int node_id) {
  if (connect()) return "";
...
BaseString NdbBackup::getNdbRestoreBinaryPath() {
  ...
  if (!File_class::exists(ndb_restore_bin_path.c_str())) {
    ndb_restore_bin_path.assfmt("%s/storage/ndb/tools/ndb_restore",
                                mysql_install_path);
    if (!File_class::exists(ndb_restore_bin_path.c_str())) {
      g_err << "Failed to find ndb_restore in either $MYSQL_BASE_DIR "
            << "or $MYSQL_BINDIR paths " << ndb_restore_bin_path.c_str()
            << endl;
      return "";
    } else
...
int NdbBackup::execRestore(bool _restore_data, bool _restore_meta,
  ...
  BaseString tmp;
  if (!isHostLocal(host)) {
    tmp.assfmt("scp -r %s:%s/BACKUP/BACKUP-%d/* .", host, path.c_str(),
               _backup_id);
  } else {
    tmp.assfmt("scp -r %s/BACKUP/BACKUP-%d/* .", path.c_str(), _backup_id);
  }
  ...
  cmd.appfmt(" -c \"%s:%d\" -n %d -b %d", ndb_mgm_get_connected_host(handle),
             ndb_mgm_get_connected_port(handle), _node_id, _backup_id);
...
int NdbBackup::restore(unsigned _backup_id, bool restore_meta,
                       bool restore_data, unsigned error_insert,
                       bool restore_epoch) {
  if (!isConnected()) return -1;
...
// Master failure
int NFDuringBackupM_codes[] = {10003, 10004, 10007, 10008,
                               10009, 10010, 10012, 10013};

// Slave failure
int NFDuringBackupS_codes[] = {10014, 10015, 10016, 10017, 10018, 10020};

// Master takeover etc...
int NFDuringBackupSL_codes[] = {10001, 10002, 10021};

int NdbBackup::NFMaster(NdbRestarter &_restarter) {
  const int sz =
      sizeof(NFDuringBackupM_codes) / sizeof(NFDuringBackupM_codes[0]);
  return NF(_restarter, NFDuringBackupM_codes, sz, true);
}

int NdbBackup::NFMasterAsSlave(NdbRestarter &_restarter) {
  const int sz =
      sizeof(NFDuringBackupS_codes) / sizeof(NFDuringBackupS_codes[0]);
  return NF(_restarter, NFDuringBackupS_codes, sz, true);
}

int NdbBackup::NFSlave(NdbRestarter &_restarter) {
  const int sz =
      sizeof(NFDuringBackupS_codes) / sizeof(NFDuringBackupS_codes[0]);
  return NF(_restarter, NFDuringBackupS_codes, sz, false);
}

int NdbBackup::NF(NdbRestarter &_restarter, int *NFDuringBackup_codes,
                  const int sz, bool onMaster) {
  int nNodes = _restarter.getNumDbNodes();
  {
    if (nNodes == 1) return NDBT_OK;
    ...
    CHECK(_restarter.startNodes(&nodeId, 1) == 0, "failed to start node");
  }
  ...
  for (int i = 0; i < sz; i++) {
    ...
    g_err << "NdbBackup::NF node = " << nodeId << " error code = " << error
          << " masterNodeId = " << masterNodeId << endl;
    ...
    CHECK(start(backupId) == 0, "failed to start backup");
...
int FailS_codes[] = {10025, 10027, 10033, 10035, 10036};

int FailM_codes[] = {10023, 10024, 10025, 10026, 10027, 10028,
                     10031, 10033, 10035, 10037, 10038};

int NdbBackup::FailMaster(NdbRestarter &_restarter) {
  const int sz = sizeof(FailM_codes) / sizeof(FailM_codes[0]);
  return Fail(_restarter, FailM_codes, sz, true);
}

int NdbBackup::FailMasterAsSlave(NdbRestarter &_restarter) {
  const int sz = sizeof(FailS_codes) / sizeof(FailS_codes[0]);
  return Fail(_restarter, FailS_codes, sz, true);
}

int NdbBackup::FailSlave(NdbRestarter &_restarter) {
  const int sz = sizeof(FailS_codes) / sizeof(FailS_codes[0]);
  return Fail(_restarter, FailS_codes, sz, false);
}

int NdbBackup::Fail(NdbRestarter &_restarter, int *Fail_codes, const int sz,
                    bool onMaster) {
  CHECK(_restarter.waitClusterStarted() == 0, "waitClusterStarted failed");
  ...
  for (int i = 0; i < sz; i++) {
    ...
    g_err << "NdbBackup::Fail node = " << nodeId << " error code = " << error
          << " masterNodeId = " << masterNodeId << endl;
...
int NdbBackup::abort(unsigned int _backup_id) {
...
int NdbBackup::set_default_encryption_password(const char *pwd, int len) {
