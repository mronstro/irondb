...
SimulatedBlock::SimulatedBlock(BlockNumber blockNumber,
                               struct Block_context &ctx, Uint32 instanceNumber)
...
void SimulatedBlock::addInstance(SimulatedBlock *b, Uint32 theInstance) {
  ...
  if (theInstanceList == 0) {
    ...
    for (Uint32 i = 0; i < MaxInstances; i++) theInstanceList[i] = 0;
  }
...
void SimulatedBlock::initCommon() {
...
void SimulatedBlock::installSimulatedBlockFunctions() {
  ...
  a[GSN_CHANGE_NODE_STATE_REQ].m_execFunction =
      &SimulatedBlock::execCHANGE_NODE_STATE_REQ;
  a[GSN_NDB_TAMPER].m_execFunction = &SimulatedBlock::execNDB_TAMPER;
  a[GSN_SIGNAL_DROPPED_REP].m_execFunction =
      &SimulatedBlock::execSIGNAL_DROPPED_REP;
  a[GSN_CONTINUE_FRAGMENTED].m_execFunction =
      &SimulatedBlock::execCONTINUE_FRAGMENTED;
  a[GSN_STOP_FOR_CRASH].m_execFunction = &SimulatedBlock::execSTOP_FOR_CRASH;
  a[GSN_UTIL_CREATE_LOCK_REF].m_execFunction =
      &SimulatedBlock::execUTIL_CREATE_LOCK_REF;
  a[GSN_UTIL_CREATE_LOCK_CONF].m_execFunction =
      &SimulatedBlock::execUTIL_CREATE_LOCK_CONF;
  a[GSN_UTIL_DESTROY_LOCK_REF].m_execFunction =
      &SimulatedBlock::execUTIL_DESTORY_LOCK_REF;
  a[GSN_UTIL_DESTROY_LOCK_CONF].m_execFunction =
      &SimulatedBlock::execUTIL_DESTORY_LOCK_CONF;
  ...
  a[GSN_SYNC_THREAD_CONF].m_execFunction =
      &SimulatedBlock::execSYNC_THREAD_CONF;
...
void SimulatedBlock::addRecSignalImpl(GlobalSignalNumber gsn, ExecFunction f,
                                      bool force) {
  if (gsn > MAX_GSN ||
      (!force && theSignalHandlerArray[gsn].m_execFunction != nullptr)) {
    char errorMsg[255];
    BaseString::snprintf(errorMsg, 255, "GSN %d(%d))", gsn, MAX_GSN);
    ERROR_SET(fatal, NDBD_EXIT_ILLEGAL_SIGNAL, errorMsg, errorMsg);
  }
  theSignalHandlerArray[gsn].m_execFunction = f;
}

void SimulatedBlock::addSignalScopeImpl(GlobalSignalNumber gsn,
                                        SignalScope scope) {
...
void SimulatedBlock::assignToThread(ThreadContext ctx) {
...
Uint32 SimulatedBlock::getInstanceNo(Uint32 nodeId, Uint32 tableId,
                                     Uint32 fragId) {
  Uint32 lqhWorkers = getNodeInfo(nodeId).m_lqh_workers;
  if (lqhWorkers == 0) return 0;
...
Uint32 SimulatedBlock::getInstanceNo(Uint32 nodeId, Uint32 instanceKey) {
  Uint32 lqhWorkers = getNodeInfo(nodeId).m_lqh_workers;
  if (lqhWorkers == 0) return 0;
  assert(instanceKey != 0);
  Uint32 instanceNo = 1 + ((instanceKey - 1) % lqhWorkers);
...
Uint32 SimulatedBlock::getInstanceKeyCanFail(Uint32 tabId, Uint32 fragId) {
  Dbdih *dbdih = (Dbdih *)globalData.getBlock(DBDIH);
  Uint32 instanceKey = dbdih->dihGetInstanceKeyCanFail(tabId, fragId);
  return instanceKey;
}

Uint32 SimulatedBlock::getInstance(Uint32 tableId, Uint32 fragId) {
...
Uint32 SimulatedBlock::getInstanceFromKey(Uint32 instanceKey) {
  Uint32 lqhWorkers = globalData.ndbMtLqhWorkers;
  Uint32 instanceNo;
  assert(instanceKey != 0);
  instanceNo = 1 + (instanceKey - 1) % lqhWorkers;
...
void SimulatedBlock::signal_error(Uint32 gsn, Uint32 len, Uint32 recBlockNo,
                                  const char *filename, int lineno) const {
  char objRef[255];
  BaseString::snprintf(objRef, 255, "%s:%d", filename, lineno);
  char probData[255];
  BaseString::snprintf(probData, 255,
                       "Signal (GSN: %d, Length: %d, Rec Block No: %d)", gsn,
                       len, recBlockNo);

  ErrorReporter::handleError(NDBD_EXIT_BLOCK_BNR_ZERO, probData, objRef);
}

extern class SectionSegmentPool g_sectionSegmentPool;

void SimulatedBlock::handle_invalid_sections_in_send_signal(
    const Signal25 *signal) const {
  char errMsg[160];
  ...
  ErrorReporter::handleError(NDBD_EXIT_BLOCK_BNR_ZERO, errMsg, "");
}

void SimulatedBlock::handle_lingering_sections_after_execute(
...
void SimulatedBlock::handle_invalid_fragmentInfo(Signal25 *signal) const {
  ErrorReporter::handleError(NDBD_EXIT_BLOCK_BNR_ZERO,
                             "Incorrect header->m_fragmentInfo in sendSignal()",
                             "");
}

void SimulatedBlock::handle_out_of_longsignal_memory(Signal25 *signal) const {
...
void SimulatedBlock::handle_send_failed(SendStatus ss, Signal25 *signal,
                                        Uint32 recNode, SecPtr ptr[]) const {
  switch (ss) {
...
static void linkSegments(Uint32 head, Uint32 tail) {
...
Uint32 getSectionSz(Uint32 id) { return g_sectionSegmentPool.getPtr(id)->m_sz; }
...
static void releaseSections(SPC_ARG Uint32 secCount,
  ...
  switch (secCount) {
    case 3:
      g_sectionSegmentPool.releaseList(SPC_SEIZE_ARG relSz(tSz2), tSec2,
                                       ptr[2].p->m_lastSegment);
      [[fallthrough]];
    case 2:
      g_sectionSegmentPool.releaseList(SPC_SEIZE_ARG relSz(tSz1), tSec1,
                                       ptr[1].p->m_lastSegment);
      [[fallthrough]];
    case 1:
      g_sectionSegmentPool.releaseList(SPC_SEIZE_ARG relSz(tSz0), tSec0,
...
void SimulatedBlock::startChangeNeighbourNode() {
...
void SimulatedBlock::setNeighbourNode(NodeId node) {
  /* We only treat neighbour nodes in a special manner in ndbmtd. */
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::endChangeNeighbourNode() {
  /* We only treat neighbour nodes in a special manner in ndbmtd. */
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::setWakeupThread(Uint32 wakeup_instance) {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::setNodeOverloadStatus(OverloadStatus new_status) {
#ifdef NDBD_MULTITHREADED
  mt_setNodeOverloadStatus(m_threadId, new_status);
#endif
}

void SimulatedBlock::setSendNodeOverloadStatus(OverloadStatus new_status) {
#ifdef NDBD_MULTITHREADED
  mt_setSendNodeOverloadStatus(new_status);
#endif
}

Uint32 SimulatedBlock::getConfiguredSpintime() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::setSpintime(Uint32 new_spintime) {
#ifdef NDBD_MULTITHREADED
  mt_setSpintime(m_threadId, new_spintime);
#endif
}

Uint32 SimulatedBlock::getWakeupLatency() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::setWakeupLatency(Uint32 latency) {
#ifdef NDBD_MULTITHREADED
  mt_setWakeupLatency(latency);
#endif
}

void SimulatedBlock::getPerformanceTimers(Uint64 &micros_sleep,
                                          Uint64 &spin_time,
                                          Uint64 &buffer_full_micros_sleep,
                                          Uint64 &micros_send) {
#ifdef NDBD_MULTITHREADED
  mt_getPerformanceTimers(m_threadId, micros_sleep, spin_time,
                          buffer_full_micros_sleep, micros_send);
#else
...
void SimulatedBlock::getSendPerformanceTimers(
    Uint32 send_instance, Uint64 &exec_time, Uint64 &sleep_time,
    Uint64 &spin_time, Uint64 &user_time_os, Uint64 &kernel_time_os,
    Uint64 &elapsed_time_os) {
  /* No send thread in ndbd */
#ifdef NDBD_MULTITHREADED
  mt_getSendPerformanceTimers(send_instance, exec_time, sleep_time, spin_time,
                              user_time_os, kernel_time_os, elapsed_time_os);
#else
  exec_time = 0;
  sleep_time = 0;
  spin_time = 0;
...
Uint32 SimulatedBlock::getNumSendThreads() {
#ifdef NDBD_MULTITHREADED
...
Uint32 SimulatedBlock::getNumThreads() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::flush_send_buffers() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::assign_recv_thread_new_trp(TrpId trp_id) {
#ifdef NDBD_MULTITHREADED
...
Uint32 SimulatedBlock::getMainThrmanInstance() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::sendSignal(BlockReference ref, GlobalSignalNumber gsn,
  ...
  if (globalData.testOn) {
    Uint16 proc = (recNode == 0 ? globalData.ownId : recNode);
    signal->header.theSendersBlockRef = sendBRef;
    globalSignalLoggers.sendSignal(signal->header, jobBuffer,
                                   &signal->theData[0], proc);
  }
#endif
...
#else
    TrpId trp_id = 0;
    ss = globalTransporterRegistry.prepareSend(
        getNonMTTransporterSendHandle(), &sh, jobBuffer, &signal->theData[0],
        recNode, trp_id, (LinearSectionPtr *)0);
#endif

    if (unlikely(!(ss == SEND_OK || ss == SEND_BLOCKED ||
...
void SimulatedBlock::sendSignal(NodeReceiverGroup rg, GlobalSignalNumber gsn,
                                Signal25 *signal, Uint32 length,
                                JobBufferLevel jobBuffer) const {
...
void SimulatedBlock::sendSignal(NodeReceiverGroup rg, GlobalSignalNumber gsn,
  ...
  if (rg.m_nodes.get(0) || rg.m_nodes.get(ourProcessor)) {
    ...
    if (globalData.testOn) {
      globalSignalLoggers.sendSignal(signal->header, jobBuffer,
                                     &signal->theData[0], ourProcessor, ptr,
                                     noOfSections);
    }
#endif
    /**
...
void SimulatedBlock::sendSignal(BlockReference ref, GlobalSignalNumber gsn,
  ...
  if (globalData.testOn) {
    ...
    signal->header.theSendersBlockRef = sendBRef;
    globalSignalLoggers.sendSignal(signal->header, jobBuffer,
                                   &signal->theData[0], proc, sections->m_ptr,
                                   noOfSections);
  }
#endif
  ...
  if (recNode == ourProcessor || recNode == 0) {
    ...
    *dst++ = sections->m_ptr[2].i;
    ...
    ss = globalTransporterRegistry.prepareSend(
        getNonMTTransporterSendHandle(), &sh, jobBuffer, &signal->theData[0],
        recNode, trp_id, g_sectionSegmentPool, sections->m_ptr);
#endif
...
void SimulatedBlock::sendSignalWithDelay(BlockReference ref,
  ...
  {
    if (globalData.testOn) {
      globalSignalLoggers.sendSignalWithDelay(
          delayInMilliSeconds, signal->header, 0, &signal->theData[0],
          globalData.ownId);
    }
  }
#endif
...
void SimulatedBlock::sendSignalWithDelay(
    BlockReference ref, GlobalSignalNumber gsn, Signal25 *signal,
...
/*
 * Copy implementation for sendSignalOverAllLinks from sendSignal, excluding
 * part for local send.
 */
void SimulatedBlock::sendSignalOverAllLinks(BlockReference ref,
                                            GlobalSignalNumber gsn,
                                            Signal25 *signal, Uint32 length,
                                            JobBufferLevel jobBuffer) const {
  BlockReference sendBRef = reference();
...
#else
  TrpBitmask trp_ids;
  ss = globalTransporterRegistry.prepareSendOverAllLinks(
      getNonMTTransporterSendHandle(), &sh, jobBuffer, &signal->theData[0],
      recNode, trp_ids);
#endif

  if (unlikely(
          !(ss == SEND_OK || ss == SEND_BLOCKED || ss == SEND_DISCONNECTED))) {
    handle_send_failed(ss, signal, recNode, (LinearSectionPtr *)NULL);
  }
  return;
}

void SimulatedBlock::release(SegmentedSectionPtr &ptr) {
  ::release(SB_SP_ARG ptr);
}

void SimulatedBlock::releaseSection(Uint32 firstSegmentIVal) {
  ::releaseSection(SB_SP_ARG firstSegmentIVal);
}

void SimulatedBlock::releaseSections(SectionHandle &handle) {
  ::releaseSections(SB_SP_ARG handle.m_cnt, handle.m_ptr);
  handle.m_cnt = 0;
}

bool SimulatedBlock::appendToSection(Uint32 &firstSegmentIVal,
                                     const Uint32 *src, Uint32 len) {
  return ::appendToSection(SB_SP_ARG firstSegmentIVal, src, len);
}

bool SimulatedBlock::import(Ptr<SectionSegment> &first, const Uint32 *src,
                            Uint32 len) {
  return ::import(SB_SP_ARG first, src, len);
}

bool SimulatedBlock::import(SegmentedSectionPtr &ptr, const Uint32 *src,
                            Uint32 len) const {
...
bool SimulatedBlock::import(SectionHandle *dst, LinearSectionPtr src[3],
                            Uint32 cnt) {
...
bool SimulatedBlock::dupSection(Uint32 &copyFirstIVal, Uint32 srcFirstIVal) {
  return ::dupSection(SB_SP_ARG copyFirstIVal, srcFirstIVal);
}

bool SimulatedBlock::writeToSection(Uint32 firstSegmentIVal, Uint32 offset,
                                    const Uint32 *src, Uint32 len) {
  return ::writeToSection(firstSegmentIVal, offset, src, len);
}

class SectionSegmentPool &SimulatedBlock::getSectionSegmentPool() {
  return g_sectionSegmentPool;
}

NewVARIABLE *SimulatedBlock::allocateBat(int batSize) {
 ...
 */
const NewVARIABLE *SimulatedBlock::getBatVar(Uint16 blockNo, Uint32 instanceNo,
                                             Uint32 varNo) {
  assert(blockNo == blockToMain(blockNo));
  /* Check blockNo in range */
  if (unlikely(blockNo > MAX_BLOCK_NO || blockNo < MIN_BLOCK_NO)) {
    assert(false);
    return NULL;
  }
...
void *SimulatedBlock::allocRecord(const char *type, size_t s, size_t n,
                                  bool clear, Uint32 paramId) {
  return allocRecordAligned(type, s, n, 0, 0, clear, paramId);
}

void *SimulatedBlock::allocRecordAligned(const char *type, size_t s, size_t n,
  ...
  if (real_size > 0) {
    ...
    g_eventLogger->info("%s::allocRecord(%s, %zu, %zu) = %llu bytes",
                        getBlockName(number()), type, s, n, real_size);
#endif
    ...
    if (p == NULL) {
      ...
      if (0 != paramId &&
          0 == ndb_mgm_get_db_parameter_info(paramId, &param_info, &size)) {
...
void SimulatedBlock::deallocRecord(void **ptr, const char *type, size_t s,
                                   size_t n) {
  (void)type;
...
int SimulatedBlock::sortchunks(const void *_e0, const void *_e1) {
  const AllocChunk *p0 = (const AllocChunk *)_e0;
  const AllocChunk *p1 = (const AllocChunk *)_e1;

  if (p0->ptrI > p1->ptrI) return 1;
  if (p0->ptrI < p1->ptrI) return -1;
  return 0;
}

Uint32 SimulatedBlock::allocChunks(AllocChunk dst[], Uint32 arraysize,
                                   Uint32 rg, Uint32 pages, Uint32 paramId) {
...
void SimulatedBlock::refresh_watch_dog(Uint32 place) {
#ifdef NDBD_MULTITHREADED
...
volatile Uint32 *SimulatedBlock::get_watch_dog() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::update_watch_dog_timer(Uint32 interval) {
                          ...
                          const char* check) const
{
  ...
  int magicStatus = (m_ctx.m_config.stopOnError() << 1) +
  ...
  if (native_strcasecmp(check, "") == 0)
    BaseString::snprintf(&buf[0], 100, "%s (Line: %d) 0x%.8x", aBlockName, line,
                         magicStatus);
  else
...
void SimulatedBlock::infoEvent(const char *msg, ...) const {
  ...
  bool ok = import(segptr, &buf_str[0], len_words);
...
#ifdef NDBD_MULTITHREADED
  sendlocal(m_threadId, &signalT.header, signalT.theData, signalT.theData + 1);
#else
...
void SimulatedBlock::execNODE_STATE_REP(Signal *signal) {
...
void SimulatedBlock::execCHANGE_NODE_STATE_REQ(Signal *signal) {
   ...
   * Pack return signal
   */
  ChangeNodeStateConf *const conf = (ChangeNodeStateConf *)&signal->theData[0];
...
void SimulatedBlock::execSIGNAL_DROPPED_REP(Signal *signal) {
  ...
  BaseString::snprintf(msg, sizeof(msg), "%s GSN: %u (%u,%u)",
                       getBlockName(number()), rep->originalGsn,
                       rep->originalLength, rep->originalSectionCount);
  ErrorReporter::handleError(NDBD_EXIT_OUT_OF_LONG_SIGNAL_MEMORY, msg, __FILE__,
                             NST_ErrorHandler);
}
...
void SimulatedBlock::execCONTINUE_FRAGMENTED(Signal *signal) {
  ...
  switch (sig->type)
  {
    ...
    case ContinueFragmented::CONTINUE_CLEANUP: {
      ...
      doNodeFailureCleanup(signal, sig->cleanup.failedNodeId,
                           sig->cleanup.resource, sig->cleanup.cursor,
                           sig->cleanup.elementsCleaned, cb);
      break;
    }
    default:
      ndbabort();
...
void SimulatedBlock::execSTOP_FOR_CRASH(Signal *signal) {
#ifdef NDBD_MULTITHREADED
  mt_execSTOP_FOR_CRASH();
#endif
}

void SimulatedBlock::execNODE_START_REP(Signal *signal) {}

void SimulatedBlock::execAPI_START_REP(Signal *signal) {}

void SimulatedBlock::execSEND_PACKED(Signal *signal) {}

ATTRIBUTE_NOINLINE
void SimulatedBlock::handle_execute_error(GlobalSignalNumber gsn) {
  /**
...
const SimulatedBlock::CallbackEntry &SimulatedBlock::getCallbackEntry(
    Uint32 ci) {
...
void SimulatedBlock::sendCallbackConf(Signal *signal, Uint32 fullBlockNo,
                                      CallbackPtr &cptr, Uint32 senderData,
                                      Uint32 callbackInfo, Uint32 returnCode) {
  ...
  if (!isNdbMtLqh()) {
    ...
    if (ce.m_flags & CALLBACK_DIRECT) {
      jam();
      EXECUTE_DIRECT_MT(blockNo, GSN_CALLBACK_CONF, signal,
                        CallbackConf::SignalLength, instanceNo);
    } else {
      jam();
      BlockReference ref = numberToRef(fullBlockNo, getOwnNodeId());
      sendSignal(ref, GSN_CALLBACK_CONF, signal, CallbackConf::SignalLength,
                 JBB);
    }
  }
...
void SimulatedBlock::execCALLBACK_CONF(Signal *signal) {
  ...
  if (ce.m_flags & CALLBACK_ACK) {
    ...
    ack->senderData = senderData;
    ack->callbackInfo = callbackInfo;
    sendSignal(senderRef, GSN_CALLBACK_ACK, signal, CallbackAck::SignalLength,
               JBB);
  }
}

#ifdef VM_TRACE_TIME
void SimulatedBlock::clearTimes() {
...
void SimulatedBlock::printTimes(FILE *output) {
  fprintf(output, "-- %s --\n", getBlockName(number()));
  Uint64 sum = 0;
  for (Uint32 i = 0; i <= MAX_GSN; i++) {
...
bool SimulatedBlock::assembleFragments(Signal *signal) {
  ...
  if (c_fragmentInfoHash.find(fragPtr, key)) {
    ...
    if (likely(!fragPtr.p->isDropped())) {
      ...
      jamBuffer()->markStartOfSigExec(signal->header.theSignalId);
 ...
 *   false if more to do
 */
bool SimulatedBlock::doCleanupFragInfo(Uint32 failedNodeId, Uint32 &cursor,
                                       Uint32 &rtUnitsUsed,
  ...
  while (!iter.isNull() && (iter.bucket == startBucket)) {
    jam();
...
bool SimulatedBlock::doCleanupFragSend(Uint32 failedNodeId, Uint32 &cursor,
                                       Uint32 &rtUnitsUsed,
                                       Uint32 &elementsCleaned) {
  jam();
  ...
  FragmentSendInfo_list *fragSendLists[NumSendLists] = {
      &c_segmentedFragmentSendList, &c_linearFragmentSendList};
...
Uint32 SimulatedBlock::doNodeFailureCleanup(Signal *signal, Uint32 failedNodeId,
                                            Uint32 resource, Uint32 cursor,
                                            Uint32 elementsCleaned,
                                            Callback &cb) {
  jam();
  const bool userCallback = (cb.m_callbackFunction != 0);
  const Uint32 maxRtUnits =
      userCallback ?
#ifdef VM_TRACE
                   2
                   :
  ...
  do {
    ...
    switch (resource) {
      case ContinueFragmented::RES_FRAGSEND: {
        jam();
        resourceDone = doCleanupFragSend(failedNodeId, cursor, rtUnitsUsed,
                                         elementsCleaned);
        break;
      }
      case ContinueFragmented::RES_FRAGINFO: {
        jam();
        resourceDone = doCleanupFragInfo(failedNodeId, cursor, rtUnitsUsed,
                                         elementsCleaned);
        break;
      }
      case ContinueFragmented::RES_LAST: {
         ...
         */
        if (userCallback) execute(signal, cb, elementsCleaned);
  ...
  constexpr Uint32 sigLen =
      ContinueFragmented::CONTINUE_CLEANUP_FIXED_WORDS + callbackWords;
...
Uint32 SimulatedBlock::simBlockNodeFailure(Signal *signal, Uint32 failedNodeId,
                                           Callback &cb) {
  jam();
  return doNodeFailureCleanup(signal, failedNodeId, 0, 0, 0, cb);
}

Uint32 SimulatedBlock::debugPrintFragmentCounts() {
  ...
  return fragmentInfoCount + linSendInfoCount + segSendInfoCount;
}
...
void SimulatedBlock::sendNextSegmentedFragment(Signal *signal,
  ...
  if (info.m_flags & FragmentSendInfo::SendNoReleaseSeg) {
    sendSignalNoRelease(info.m_nodeReceiverGroup, info.m_gsn, signal,
                        sigLen + secCount + 1, (JobBufferLevel)info.m_prio,
                        &sections);
    ...
    sendSignal(info.m_nodeReceiverGroup, info.m_gsn, signal,
               sigLen + secCount + 1, (JobBufferLevel)info.m_prio, &sections);
  }
...
bool SimulatedBlock::sendFirstFragment(
...
void SimulatedBlock::sendFragmentedSignal(BlockReference ref,
  ...
  res = sendFirstFragment(*tmp.p, NodeReceiverGroup(ref), gsn, signal, length,
                          jbuf, sections,
  ...
  if (tmp.p->m_status == FragmentSendInfo::SendComplete) {
    ...
    if (c.m_callbackFunction != 0) execute(signal, c, 0);
    return;
  }
  tmp.p->m_callback = c;
...
void SimulatedBlock::sendFragmentedSignal(NodeReceiverGroup rg,
  ...
  res = sendFirstFragment(*tmp.p, rg, gsn, signal, length, jbuf, sections,
                          false,  // Release sections on send
                          messageSize);
  ndbrequire(res);
...
void SimulatedBlock::TheNULLCallbackFunction(class Signal *, Uint32, Uint32) {
  abort(); /* should never be called */
}
SimulatedBlock::Callback SimulatedBlock::TheNULLCallback = {
    &SimulatedBlock::TheNULLCallbackFunction, 0};

void SimulatedBlock::sendFragmentedSignal(
  ...
  res = sendFirstFragment(*tmp.p, NodeReceiverGroup(ref), gsn, signal, length,
                          jbuf, ptr, noOfSections, messageSize);
  ndbrequire(res);
...
void SimulatedBlock::sendFragmentedSignal(
  ...
  res = sendFirstFragment(*tmp.p, rg, gsn, signal, length, jbuf, ptr,
                          noOfSections, messageSize);
  ndbrequire(res);
...
void SimulatedBlock::sendBatchedFragmentedSignal(
    BlockReference ref, GlobalSignalNumber gsn, Signal *signal, Uint32 length,
    JobBufferLevel jbuf, SectionHandle *sections, bool noRelease, Callback &c,
    Uint32 messageSize) {
  jam();
  bool res = true;
  ...
  const Uint32 totalSize = (noOfSections >= 1 ? ptr[0].sz : 0) +
                           (noOfSections >= 2 ? ptr[1].sz : 0) +
                           (noOfSections >= 3 ? ptr[2].sz : 0);

  res = sendFirstFragment(fragSendInfo, NodeReceiverGroup(ref), gsn, signal,
                          length, jbuf, sections, noRelease, messageSize);
  ndbrequire(res);
...
void SimulatedBlock::sendBatchedFragmentedSignal(
  ...
  res = sendFirstFragment(fragSendInfo, rg, gsn, signal, length, jbuf, sections,
                          noRelease, messageSize);
  ndbrequire(res);
...
void SimulatedBlock::sendBatchedFragmentedSignal(
  ...
  res = sendFirstFragment(fragSendInfo, NodeReceiverGroup(ref), gsn, signal,
                          length, jbuf, ptr, noOfSections, messageSize);
  ndbrequire(res);
...
void SimulatedBlock::sendBatchedFragmentedSignal(
  ...
  res = sendFirstFragment(fragSendInfo, rg, gsn, signal, length, jbuf, ptr,
                          noOfSections, messageSize);
  ndbrequire(res);
...
NodeInfo &SimulatedBlock::setNodeInfo(NodeId nodeId) {
...
void SimulatedBlock::ignoreMutexUnlockCallback(Signal *signal, Uint32 ptrI,
                                               Uint32 retVal) {
  c_mutexMgr.release(ptrI);
}

void SimulatedBlock::fsRefError(Signal *signal, Uint32 line, const char *msg) {
  const FsRef *fsRef = (FsRef *)signal->getDataPtr();
  Uint32 errorCode = fsRef->errorCode;
  Uint32 osErrorCode = fsRef->osErrorCode;
  char msg2[100];

  sprintf(msg2, "%s: %s. OS errno: %u", getBlockName(number()), msg,
          osErrorCode);

  progError(line, errorCode, msg2);
}

void SimulatedBlock::execFSWRITEREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system write failed");
}

void SimulatedBlock::execFSREADREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system read failed");
}

void SimulatedBlock::execFSCLOSEREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system close failed");
}

void SimulatedBlock::execFSOPENREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system open failed");
}

void SimulatedBlock::execFSREMOVEREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system remove failed");
}

void SimulatedBlock::execFSSYNCREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system sync failed");
}

void SimulatedBlock::execFSAPPENDREF(Signal *signal) {
  fsRefError(signal, __LINE__, "File system append failed");
}

#if defined(USE_INIT_GLOBAL_VARIABLES)
void SimulatedBlock::disable_global_variables() {
#ifdef NDBD_MULTITHREADED
  mt_disable_global_variables(m_threadId);
#endif
}

void SimulatedBlock::enable_global_variables() {
#ifdef NDBD_MULTITHREADED
  mt_enable_global_variables(m_threadId);
#endif
}

void SimulatedBlock::init_global_ptrs(void **tmp, size_t cnt) {
#ifdef NDBD_MULTITHREADED
  mt_init_global_variables_ptr_instances(m_threadId, tmp, cnt);
#endif
}

void SimulatedBlock::init_global_uint32_ptrs(void **tmp, size_t cnt) {
#ifdef NDBD_MULTITHREADED
  mt_init_global_variables_uint32_ptr_instances(m_threadId, tmp, cnt);
#endif
}

void SimulatedBlock::init_global_uint32(void **tmp, size_t cnt) {
#ifdef NDBD_MULTITHREADED
...
int SimulatedBlock::cmp_key(Uint32 tab, const Uint32 *s1,
  ...
  for (Uint32 i = 0; i < noOfKeyAttr; i++) {
    ...
    const int res =
        cmp_attr(attrDesc, keyAttr.charsetInfo, s1, srcBytes, s2, srcBytes);
    if (res != 0) return res;
...
int SimulatedBlock::cmp_attr(Uint32 attrDesc, const CHARSET_INFO *cs,
                             const Uint32 *s1, Uint32 s1Len, const Uint32 *s2,
                             Uint32 s2Len) const {
  const Uint32 typeId = AttributeDescriptor::getType(attrDesc);
 ...
 */
Uint32 SimulatedBlock::xfrm_key_hash(
    Uint32 tab, const Uint32 *src, Uint32 *dst, Uint32 dstSize,
  ...
  while (i < noOfKeyAttr) {
    const KeyDescriptor::KeyAttr &keyAttr = desc->keyAttr[i];
    Uint32 dstWords =
        xfrm_attr_hash(keyAttr.attributeDescriptor, keyAttr.charsetInfo, src,
                       srcPos, dst, dstPos, dstSize);
    keyPartLen[i++] = dstWords;
    if (unlikely(dstWords == 0)) return 0;
  }
...
Uint32 SimulatedBlock::xfrm_attr_hash(Uint32 attrDesc, const CHARSET_INFO *cs,
                                      const Uint32 *src, Uint32 &srcPos,
                                      Uint32 *dst, Uint32 &dstPos,
                                      Uint32 dstSize) const {
  ...
  if (cs == NULL) {
    ...
    if (0) {
      ...
      for (Uint32 i = 0; i < srcWords; i++) printf("%.8x ", src[srcPos + i]);
      printf("\n");
    }
  } else {
    jam();
    ...
    int n = NdbSqlUtil::strnxfrm_hash(cs, typeId, dstPtr, remLen, srcPtr + lb,
                                      len, defLen);
...
Uint32 SimulatedBlock::create_distr_key(
    Uint32 tableId, const Uint32 *src, Uint32 *dst,
    const Uint32 keyPartLen[MAX_ATTRIBUTES_IN_INDEX]) const {
  ...
  if (keyPartLen) {
    ...
    while (i < noOfKeyAttr && noOfDistrKeys) {
      ...
      ndbrequire(AttributeDescriptor::getArrayType(attr) ==
                 NDB_ARRAYTYPE_FIXED);
      if (AttributeDescriptor::getDKey(attr)) {
        noOfDistrKeys--;
        memmove(dst + dstPos, src, len << 2);
...
void SimulatedBlock::sendRoutedSignal(RoutePath path[], Uint32 pathcnt,
                                      Uint32 dst[], Uint32 dstcnt, Uint32 gsn,
                                      Signal *signal, Uint32 sigLen,
                                      JobBufferLevel prio,
  ...
  for (Uint32 i = 0; i < dstcnt; i++) {
    ndbrequire(refToNode(dst[i]) == 0 || refToNode(dst[i]) == getOwnNodeId());

    *dstptr++ = dst[i];
  }

  sendSignal(path[0].ref, GSN_LOCAL_ROUTE_ORD, signal, len, path[0].prio,
             &handle);
}
...
void SimulatedBlock::execLOCAL_ROUTE_ORD(Signal *signal) {
  ...
  ndbrequire(sigLen >= (LocalRouteOrd::StaticLen + (pathcnt * 2) + (dstcnt)));
  ...
  if (pathcnt == 0) {
    ...
    static_assert(25 + LocalRouteOrd::MaxDstCount <=
    ...
    sendSignal(ref, GSN_LOCAL_ROUTE_ORD, signal, len, JobBufferLevel(prio),
               &handle);
  }
}
...
const char *SimulatedBlock::debugOutTag(char *buf, int line) {
  char blockbuf[40];
  char instancebuf[40];
  char linebuf[40];
  char timebuf[40];
...
void SimulatedBlock::synchronize_threads(Signal *signal,
...
void SimulatedBlock::sendSYNC_THREAD_REQ(Signal *signal,
                                         Ptr<SyncThreadRecord> ptr) {
...
void SimulatedBlock::execSYNC_THREAD_REQ(Signal *signal) {
  jamEntry();
...
void SimulatedBlock::execSYNC_THREAD_CONF(Signal *signal) {
  jamEntry();
  Ptr<SyncThreadRecord> ptr;
...
void SimulatedBlock::execSYNC_REQ(Signal *signal) {
  jamEntry();
  Uint32 ref = signal->theData[0];
...
void SimulatedBlock::synchronize_external_signals(Signal *signal,
                                                  const Callback &cb) {
#ifndef NDBD_MULTITHREADED
  Callback copy = cb;
  execute(signal, copy, 0);
...
void SimulatedBlock::synchronize_path(Signal *signal, const Uint32 blocks[],
                                      const Callback &cb, JobBufferLevel prio) {
  jam();
  ...
  if (blocks[0] == 0) {
    ...
    sendSignal(numberToRef(blocks[0], getOwnNodeId()), GSN_SYNC_PATH_REQ,
               signal, SyncPathReq::SignalLength + (1 + len), prio);
  }
}

void SimulatedBlock::execSYNC_PATH_REQ(Signal *signal) {
  LOCAL_SIGNAL(signal);
  jamEntry();
  SyncPathReq *req = CAST_PTR(SyncPathReq, signal->getDataPtrSend());
...
void SimulatedBlock::execSYNC_PATH_CONF(Signal *signal) {
  jamEntry();
...
bool SimulatedBlock::checkNodeFailSequence(Signal *signal) {
   ...
   */
  if (ref == reference() ||
      (refToNode(ref) == getOwnNodeId() && refToMain(ref) == DBDIH)) {
    jam();
    return true;
  }
...
#ifdef ERROR_INSERT
void SimulatedBlock::setDelayedPrepare() {
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::ndbinfo_send_row(Signal *signal, const DbinfoScanReq &req,
  ...
  sendSignal(req.resultRef, GSN_DBINFO_TRANSID_AI, signal,
             TransIdAI::HeaderLength, JBB, ptr, 1);
}

void SimulatedBlock::ndbinfo_send_scan_break(Signal *signal, DbinfoScanReq &req,
  ...
  if (rl.rows > 0) {
    ...
    sendSignal(reference(), GSN_DBINFO_SCANREQ, signal, signal_length, JBB);
  }
}

void SimulatedBlock::ndbinfo_send_scan_conf(
    Signal *signal, DbinfoScanReq &req, const Ndbinfo::Ratelimit &rl) const {
  DbinfoScanConf *conf = (DbinfoScanConf *)signal->getDataPtrSend();
  const Uint32 signal_length = DbinfoScanReq::SignalLength + req.cursor_sz;
  ...
  sendSignal(sender_ref, GSN_DBINFO_SCANCONF, signal, signal_length, JBB);
}
...
*/
Uint64 SimulatedBlock::elapsed_time(Signal *signal, const NDB_TICKS currentTime,
                                    NDB_TICKS &latestTIME_SIGNAL,
...
Uint32 SimulatedBlock::get_recv_thread_idx(TrpId trp_id) {
#ifdef NDBD_MULTITHREADED
...
SectionSegment *SimulatedBlock::getSegmentPtr(Uint32 iVal) {
  return g_sectionSegmentPool.getPtr(iVal);
}

bool SimulatedBlock::seizeSegment(Ptr<SectionSegment> &p) {
  return g_sectionSegmentPool.seize(SB_SP_REL_ARG p);
}

void SimulatedBlock::releaseSegment(Uint32 iVal) {
  g_sectionSegmentPool.release(SB_SP_REL_ARG iVal);
}

void SimulatedBlock::releaseSegmentList(Uint32 firstSegmentIVal) {
  ::releaseSection(SB_SP_ARG firstSegmentIVal);
}

#ifdef NDB_DEBUG_RES_OWNERSHIP
void SimulatedBlock::lock_global_ssp() {
#ifdef NDBD_MULTITHREADED
  f_section_lock.lock();
#endif
}

void SimulatedBlock::unlock_global_ssp() {
#ifdef NDBD_MULTITHREADED
...
#endif
#endif
void SimulatedBlock::print_debug_sched_stats(
  ...
  for (Uint32 j = 1; j <= globalData.ndbMtQueryWorkers; j++)
  {
    ...
    g_eventLogger->info("QT(%u) total_signals: %llu, total_words: %llu, "
                        ...
                        "%llu %llu %llu %llu "
                        "%llu %llu %llu %llu ",
                        j, *total_signals, *total_words,
                        est_stat[0], est_stat[1], est_stat[2], est_stat[3],
                        est_stat[4], est_stat[5], est_stat[6], est_stat[7],
                        est_stat[8], est_stat[9], est_stat[10],est_stat[11],
                        est_stat[12],est_stat[13],est_stat[14],est_stat[15]);
    *total_words = 0;
    *total_signals = 0;
...
void SimulatedBlock::get_load_indicators(DistributionHandler *const handle,
                                         Uint32 rr_group) {
#ifdef NDBD_MULTITHREADED
 ...
 * query thread or an LDM thread. Since RonDB 21.10 and included in
 * new LTS version RonDB 22.10 we also send LQHKEYREQ for locked reads
 * to either an LDM or Query block.
 * Previous to version RonDB 22.10 the query blocks
 * executed in their own thread. In RonDB this changed such that each
 * LDM thread contains both blocks for LDM execution and blocks for
 * query execution.
 *
 * This code makes the selection of where to send it and returns a
 * block reference where the signal should be sent.
 *
 ...
 * well spawn up and complete within one 100 ms interval. Thus it would
 ...
 * 1. Scheduling
 * -------------
 * The principle used is fairly simple. The THRMAN will decide the work
 * balance to use for the next 100 ms. The work balance is sent with
 * an array of weights. These weights range between 1 and 16. At the
 * start of each such new interval each thread gets a new weight.
 *
 * The execution starts with assigning each thread its weight in
 * distribute_new_weights.
 * If the weight is e.g. 10, it means that we can execute 10
 * LQHKEYREQ signals at normal load. If load increases this will
 * consume those 10 weights faster. We will first try to use the
 * LDM thread where data resides, next the query block in the LDM
 * thread executing on the same hyperthread. Finally if both of
 * those has already "stolen" their weights we will follow a pattern
 * that directs us to use the remaining LDM threads in a pattern
 * that makes most use of LDM threads with higher weight.
 *
 * We track the number of signals sent and each time the counter
 * comes to 48 (a key requests adds 1 and a scan requests adds 5)
 * we will check the load indicators of all other threads and set
 * new load indicators. The load indicators are relative, so if
 * all threads are overloaded, the least loaded will still have
 * load 1.
 * 
 ...
 *
 * Scans consume 4 weights instead, to handle this we multiply the
 * weight we receive by 4.
 */
Uint32 SimulatedBlock::get_lqhkeyreq_ref(DistributionHandler *const handle,
...
#endif
  {
    ...
    for (Uint32 i = 0; i < num_lqhs; i++) {
      ...
      Uint32 load_indicator = q_state->m_load_indicator;
      Uint32 current_stolen = q_state->m_current_stolen;
      jamDebug();
  ...
  do {
    ...
    if (current_stolen < weight) {
      ...
      jamDataDebug(load_indicator);
      /**
       * This query thread haven't stolen all weights yet, so use it to
...
#endif
  {
    ...
    for (Uint32 i = 0; i < num_lqhs; i++) {
      struct QueryThreadState *q_state =
        &handle->m_query_state[query_instance_no - 1];
      Uint32 weight = q_state->m_weight;
      Uint32 load_indicator = q_state->m_load_indicator * LOAD_SCAN_FRAGREQ;
      Uint32 current_stolen = q_state->m_current_stolen;
      jamDebug();
      jamDataDebug(query_instance_no);
      jamDataDebug(current_stolen);
 ...
 */
Uint32 SimulatedBlock::m_shared_ldm_instance[MAX_DISTR_THREADS];
