...
bool Dbtup::readTriggerInfo(TupTriggerData *const trigPtr,
  ...
  if (regOperPtr->op_type == ZDELETE && !regOperPtr->is_first_operation()) {
     ...
     * is always the original one, so its header will never contain
     * `DISK_INLINE`.
     */
    jam();
    req_struct->m_tuple_ptr =
  ...
  if (regTabPtr->need_expand(disk)) {
    /*
     * MTR case: bug_rondb-852.test, see 'CASE 2'
     *
     * `prepare_read` can handle both original and copy tuples,
     * distinguishing them by checking whether their header bits contain
     * the `DISK_INDEX` flag.
     *
     * For an original tuple, `req_struct->m_disk_page_ptr.i` must be used,
     * so we need to set it to `regOperPtr->m_disk_callback_page`.
     * For a copy tuple, `req_struct->m_disk_page_ptr.i` is not actually needed.
     * Consider the following scenario:
     *
     * BEGIN;
     * UPDATE (causes page expansion);
     * DELETE;
     * COMMIT;
     *
     * During the commit phase, we commit the delete operation.
     * The tuple at this stage is expected to be a copy tuple (generated
     * by the update operation and containing `DISK_INLINE`). However, due
     * to the dead code above, the tuple is instead the original one,
     * which does not contain `DISK_INLINE`.
     *
     * But `req_struct->m_disk_page_ptr.i` is pointing to the extra page now.
     * To ensure `prepare_read()` functions correctly, we need to overwrite
     * `req_struct->m_disk_page_ptr.i` to point to the original page.
     */
  ...
  if ((regOperPtr->op_type == ZUPDATE || regOperPtr->op_type == ZDELETE) &&
    ...
    if (regTabPtr->need_expand(disk)) {
      /*
       * MTR case: bug_rondb-852.test, see 'CASE 1'
       *
       * If regOperPtr->m_disk_extra_callback_page is not RNIL,
