...
bool Dbtup::readTriggerInfo(TupTriggerData *const trigPtr,
  ...
  if (regTabPtr->need_expand(disk)) {
     ...
     * distinguishing them by checking whether their header bits contain the `DISK_INDEX` flag.
     *
     ...
     * The tuple at this stage is expected to be a copy tuple (generated by the update operation
     * and containing `DISK_INLINE`). However, due to the dead code above,
     * the tuple is instead the original one, which does not contain `DISK_INLINE`.
