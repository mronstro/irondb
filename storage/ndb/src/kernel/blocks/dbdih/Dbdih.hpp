...
class Dbdih : public SimulatedBlock {
 ...
 public:
  ...
  struct TabRecord {
    ...
    enum TableFlags { TF_FULLY_REPLICATED = 1 };
 ...
 private:
  ...
  void calculate_most_recent_node(
  ...
  void pause_lcp(Signal *signal, Uint32 startNode, BlockReference sender_ref);
  void unpause_lcp(Signal *signal, Uint32 startNode, BlockReference sender_ref,
   ...
   */
  bool is_lcp_paused() { return (c_pause_lcp_start_node != RNIL); }
 ...
 private:
  ...
  bool verify_fragmentation(Uint16 *fragments, Uint32 partition_count,
                            Uint32 partition_balance, Uint32 ldm_count) const;
  ...
  void sendCopyTable(Signal *, CopyTableNode *ctn, BlockReference ref,
                     Uint32 reqinfo);
  void sendDihfragreq(Signal *, TabRecordPtr regTabPtr, Uint32 fragId);
  ...
  void start_add_fragments_in_new_table(TabRecordPtr, ConnectRecordPtr,
                                        const Uint16 buf[], const Uint32 bufLen,
                                        Signal *signal);
  void make_new_table_writeable(TabRecordPtr, ConnectRecordPtr, bool);
  void make_new_table_read_and_writeable(TabRecordPtr, ConnectRecordPtr,
                                         Signal *);
...
//------------------------------------
  ...
  void replication(Uint32 noOfReplicas, NodeGroupRecordPtr NGPtr,
  ...
  void readReplicas(RWFragment *rf, TabRecord *regTabPtr,
                    FragmentstorePtr regFragptr);
  void updateLcpInfo(TabRecord *regTabPtr, Fragmentstore *regFragPtr,
  ...
  void addtabrefuseLab(Signal *, ConnectRecordPtr regConnectPtr,
                       Uint32 errorCode);
  ...
  void breakCopyTableLab(Signal *, TabRecordPtr regTabPtr, Uint32 nodeId);
  void checkAddfragCompletedLab(Signal *, TabRecordPtr regTabPtr,
                                Uint32 fragId);
  ...
  void rnfTableNotReadyLab(Signal *, TabRecordPtr regTabPtr,
                           Uint32 removeNodeId);
  ...
  void copyTableNode(Signal *, CopyTableNode *ctn, NodeRecordPtr regNodePtr);
  ...
  void setGCPStopTimeouts(Signal *, bool set_gcp_save_max_lag = true,
  ...
  void findReplica(ReplicaRecordPtr &regReplicaPtr, Fragmentstore *fragPtrP,
                   Uint32 nodeId, bool oldStoredReplicas = false);
  //------------------------------------
...
//------------------------------------
  ...
  //------------------------------------
  void insertCopyFragmentList(TabRecord *tabPtr, Fragmentstore *fragPtr,
                              Uint32 my_fragid);
  void allocStoredReplica(FragmentstorePtr regFragptr,
                          ReplicaRecordPtr &newReplicaPtr, Uint32 nodeId,
                          Uint32 fragId, Uint32 tableId);
  ...
  bool findBestLogNode(CreateReplicaRecord *createReplica,
                       FragmentstorePtr regFragptr, Uint32 startGci,
                       Uint32 stopGci, Uint32 logNode, Uint32 &fblStopGci);
  ...
  void removeNodeFromStored(Uint32 nodeId, FragmentstorePtr regFragptr,
                            ReplicaRecordPtr replicaPtr, bool temporary);
...
//------------------------------------
   ...
   */
  Uint16 tmp_next_replica_node[MAX_NDB_NODE_GROUPS][NDBMT_MAX_WORKER_INSTANCES];
...
#define ZMAX_TAKE_OVER_THREADS 64
  ...
  //-----------------------------------------------------
  void startTakeOver(Signal *, Uint32 startNode, Uint32 toNode,
  ...
  void init_takeover_thread(TakeOverRecordPtr takeOverPtr,
                            ...
                            Uint32 number_of_threads, Uint32 thread_id);
  ...
  void send_continueb_nr_start_logging(Signal *signal,
                                       TakeOverRecordPtr takeOverPtr);
  bool thread_takeover_completed(Signal *signal, TakeOverRecordPtr takeOverPtr);
  ...
  void sendUpdateFragStateReq(Signal *, Uint32 startGci, Uint32 storedType,
                              TakeOverRecordPtr takeOverPtr);
  ...
  */
  ...
  struct CopyGCIMaster {
    CopyGCIMaster() {
      ...
      for (Uint32 i = 0; i < WAIT_CNT; i++) m_waiting[i] = CopyGCIReq::IDLE;
    }
    /*------------------------------------------------------------------------*/
 ...
 private:
  struct LcpState {
    ...
    void setLcpStatus(LcpStatus status, Uint32 line) {
      for (Uint32 i = 9; i > 0; i--) m_saveState[i] = m_saveState[i - 1];
 ...
 private:
  ...
  bool getBlockCommit() const { return c_blockCommit || cgckptflag; }
  ...
  void switchReplica(Signal *, Uint32 nodeId, Uint32 tableId, Uint32 fragNo);
  ...
  void getNodeBitmap(NdbNodeBitmask &map, Uint32 listHead,
...
#define DIH_CDATA_SIZE _SYSFILE_FILE_SIZE
...
#ifdef ERROR_INSERT
  void sendToRandomNodes(const char *, Signal *, SignalCounter *, SendFunction,
                         Uint32 extra = RNIL, Uint32 block = 0, Uint32 gsn = 0,
                         Uint32 len = 0, JobBufferLevel = JBB);
#endif
  ...
  struct LocalLCPState {
    ...
    Uint32
        m_stop_gci;  // This GCI needs to be complete before LCP is restorable
  ...
  Uint32 getFragmentCount(Uint32 partitionBalance, Uint32 numOfNodeGroups,
                          Uint32 numOfReplicas, Uint32 numOfLDMs) const;
  /**
   * dihGetInstanceKey
   *
  ...
  void isolateNodes(Signal *signal, Uint32 delayMillis,
